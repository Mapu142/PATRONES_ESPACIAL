...
grd <- expand.grid(
  x = seq(bb["xmin"], bb["xmax"], length.out = 100),
  y = seq(bb["ymin"], bb["ymax"], length.out = 100)
)
...
grd <- st_as_sf(
  grd,
  coords = c("x", "y"),
  crs = st_crs(datos_modelo)
)

# Recuperar las coordenadas como columnas
coord <- st_coordinates(grd)
grd$x <- coord[,1]
grd$y <- coord[,2]

# Krigin universal
kriged_values_universal <- krige(
  formula = PM10 ~ x,
  locations = datos_modelo,
  newdata = grd,
  model = modelo_teorico_universal
)

bogota_4326 <- st_transform(
  bogota,
  st_crs(kriged_values_universal)
)

kriged_values_universal <- st_intersection(
  kriged_values_universal,
  bogota_4326
)
...
p1 <- ggplot() +
  ...
  labs(title = "Predicción mediante Kriging Universal") +
  theme_bw()
...
p2 <- ggplot() +
  ...
  labs(title = "Varianza de la predicción") +
  theme_bw()

p1 + p2
```

En el mapa de predicción se observa que la mayor parte del área de estudio presenta valores bajos o medios de PM10, con concentraciones más elevadas hacia el occidente de la ciudad. Por otra parte, el mapa de la varianza de predicción muestra un comportamiento esperado del kriging universal: la incertidumbre aumenta a medida que los puntos de predicción se alejan de las estaciones de monitoreo, debido a la menor disponibilidad de información para realizar la interpolación. En contraste, en las ubicaciones cercanas o coincidentes con una estación de monitoreo la varianza de predicción es considerablemente menor, ya que en estos puntos la estimación está fuertemente respaldada por observaciones directas. Este patrón evidencia la dependencia de la precisión del predictor respecto a la distribución espacial de las estaciones.

Resulta particularmente interesante observar un punto de muy baja incertidumbre en el centro-occidente del mapa, el cual coincide con la ubicación de una estación de monitoreo. En este caso, el predictor prácticamente reproduce el valor observado, por lo que la incertidumbre es mínima en comparación con las zonas circundantes.
