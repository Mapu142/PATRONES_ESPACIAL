...
Para que la estimación del semivariograma sea válida, debe calcularse sobre el proceso centrado $Z(s)$ (los residuales del modelo de tendencia), garantizando así la estacionariedad en la media. 

Por otro lado dado que el semivariograma empírico $\hat{\gamma}(h)$ estima la varianza de los incrementos espaciales en función de la distancia euclidiana $\|h\|$ entre pares de observaciones. Para que esta distancia sea métricamente válida y físicamente interpretable, es necesario trabajar en un Sistema de Coordenadas Planas (Proyectadas).


Para el área de estudio de Bogotá D.C., el sistema de coordenadas planas oficial y más adecuado es el UTM (Universal Transverse Mercator) Zona 18 Norte, con datum WGS84 (código *EPSG: 32618*). En este sistema, las unidades son **metros**, lo que garantiza que el "rango" (alcance espacial) y la "silla" (varianza) del modelo teórico tengan una interpretación física real.

 Dado que el número total de pares de datos es limitado ($N_p = 153$), se establece una distancia máxima de corte (`max.dist = 15000` metros) para evitar la inestabilidad del estimador a grandes distancias donde la cantidad de pares decae drásticamente.
