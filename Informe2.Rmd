...
```{r configuracion-fecha, message=FALSE, warning=FALSE, include=FALSE}
...
Debido a la amplia extensión territorial de las localidades de Usme y Ciudad Bolívar hacia el sur y a la presencia de una sola estación de monitoreo en dicha zona, se fijó un límite latitudinal sur a los $4.51^\circ\text{N}$. Esta delimitación espacial evita distorsiones asociadas al efecto de borde y a la baja densidad muestral, garantizando una estimación robusta del semivariograma y previniendo artefactos en la interpolación por kriging.


```{r faltantes, include=FALSE, tab.cap="Conteo de datos faltantes"}
...
```{r mapa-bogota, message=FALSE, warning=FALSE}
colores <- list(
  azul_oscuro = "#2c3e50",
  azul_medio = "#34495e",
  gris_suave = "#f0f0f0",
...
# Exploración espacial
 ...
 De acuerdo con el modelo geoestadístico, el proceso espacial de interés se descompone en una componente determinística (tendencia) y una estocástica (proceso centrado):
$$ Y(s) = \mu(s) + Z(s) $$
Donde $E[Z(s)] = 0$. Para que la geoestadística (la estimación del semivariograma y el Kriging) sean válidas, el proceso $Z(s)$ debe cumplir con estacionariedad de segundo orden o intrínseca. 
...
```{r boxplot-pm10, fig.cap="Diagrama de caja de la variable PM10", fig.height=2.5, fig.width=8, message=FALSE, warning=FALSE}
ggplot()+
...
```{r exploratorio-p10, echo=FALSE, fig.align="center", fig.cap="Distribución empírica de la concentración de PM10.", fig.height=2.5, fig.width=10}
...
```{r dispm10lon, fig.height=2.5, fig.width=8, message=FALSE, warning=FALSE}
ggplot(datos_corte,
       aes(Longitud, PM10))+
    geom_point(alpha=.3)+
...
```{r disPM10lat, fig.height=2.5, fig.width=8, message=FALSE, warning=FALSE}
ggplot(datos_corte,
       aes(Latitud, PM10))+
    geom_point(alpha=.3)+
    geom_smooth(method="loess")
...
El análisis exploratorio previo (gráficos de dispersión con *Loess*) evidenció que la concentración de $\text{PM}_{10}$ no es constante en el espacio, sino que presenta gradientes significativos a lo largo de las coordenadas geográficas (Longitud $x$ y Latitud $y$). Por lo tanto, es necesario modelar la media $\mu(s)$ mediante regresión espacial, de modo que los residuales $\hat{Z}(s) = Y(s) - \hat{\mu}(s)$ constituyan un proceso estacionario de media cero, libre de tendencia espacial.
...
Basado en la topografía de Bogotá (que genera gradientes de contaminación tanto de Occidente a Oriente por los Cerros, como de Sur a Norte), se proponen cinco modelos de regresión polinómica para capturar la tendencia $\mu(s)$:

1. **Lineal Múltiple (`x + y`):** Tendencia plana en ambas direcciones (Modelo de primer orden).
2. **Cuadrático (`x + y + I(x^2) + I(y^2) + I(x*y)`):** Superficie curva (Modelo de segundo orden).
3. **Interacción (`x * y`):** Superficie plana con pendiente variable (Modelo de primer orden con interacción).
4. **Mixto (`x + I(y^2)`):** Tendencia lineal en un eje y cuadrática en el otro.
5. **Lineal Simple (`x`):** Tendencia unidireccional.

A continuación, se ajustan los modelos candidatos y se evalúan mediante tres criterios: (AIC) (donde menor valor indica mejor ajuste penalizado por complejidad), el $R^2$ Ajustado (proporción de varianza espacial explicada) y la Prueba de Shapiro-Wilk (para validar la normalidad de los residuales, requisito para la estimación de verosimilitud del variograma).
...
datos_modelo <- datos_corte %>%
 ...
 st_as_sf(coords = c("x", "y"), crs = 32618, remove = FALSE) # Convertimos a sf para mapeo de residuos
...
Al evaluar los resultados de la Tabla de Análisis de Varianza (ANOVA) y el resumen del modelo de regresión, se observa que únicamente la variable espacial $x$ (longitud) resulta ser estadísticamente significativa ($F = 11.05, p = 0.00402$).Por lo tanto, atendiendo al principio de parsimonia (preferir modelos simples y explicativos frente a configuraciones complejas que no aportan significancia), se seleccionó el modelo lineal simple basado únicamente en la coordenada $x$ para capturar la tendencia de primer orden de $\text{PM}_{10}$.




```{r ANOVA_Modelo_Seleccionado, echo=FALSE}
# Validación estadística del modelo seleccionado
knitr::kable(car::Anova(mod_5, type = "II"), 
             caption = "Análisis de Varianza (ANOVA) para el modelo Lineal Múltiple",
             digits = 4)
```

El ANOVA confirma que la coordenada $x$ (Longitud) es un  predictor significativo ($p < 0.01$) de la concentración de $\text{PM}_{10}$.


Una vez seleccionado el modelo, se verifica que la tendencia ha sido eliminada. Si el modelo es adecuado, los residuales $\hat{Z}(s)$ no deben mostrar patrones espaciales.

```{r Diagnostico_Residuales, fig.height=3, fig.width=10, message=FALSE, warning=FALSE}
# Extraer residuales del modelo ganador
datos_modelo$resid_final <- residuals(mod_5)

# Función para graficar residuales vs coordenadas
plot_residuos <- function(datos, var_residuo, titulo) {
  p1 <- ggplot(datos, aes(x = x, y = .data[[var_residuo]])) +
    ...
    labs(title = titulo, x = "Longitud (x)", y = expression(hat(Z)(s))) +
  ...
  p2 <- ggplot(datos, aes(x = y, y = .data[[var_residuo]])) +
    ...
    geom_smooth(method = "lm", color = "red", se = FALSE, linetype = "dashed") +
    geom_hline(yintercept = 0, color = "black", linetype = "solid") +
    labs(title = "", x = "Latitud (y)", y = expression(hat(Z)(s))) +
    theme_minimal()
...
plot_residuos(st_drop_geometry(datos_modelo), "resid_final", 
              "Diagnóstico: Residuales del Modelo Lineal Simple (x)")
```

En los gráficos de diagnóstico se observa que la línea de tendencia roja (ajuste lineal a los residuales) es prácticamente horizontal y se superpone con la línea negra en $y=0$. Esto demuestra que la esperanza condicional de los residuales es cero ($E[\hat{Z}(s)|x,y] \approx 0$), cumpliendo con el supuesto de estacionariedad en la media. A partir de este punto, el semivariograma empírico se calculará utilizando estos residuales $\hat{Z}(s)$, garantizando que la variabilidad estimada responda únicamente a la autocorrelación espacial y no a la tendencia determinística.
...
Para que la estimación del semivariograma sea válida y sus parámetros (alcance y meseta) tengan una interpretación física real, es necesario trabajar sobre el proceso centrado $\hat{Z}(s)$ (los residuales del modelo de tendencia) y utilizar un Sistema de Coordenadas Planas (Proyectadas).



De acuerdo con la teoría de campos aleatorios espaciales (Bohórquez, 2025, Cap. 2; Baragatt et al., 2019, Apéndice B), el semivariograma empírico $\hat{\gamma}(h)$ estima la varianza de los incrementos en función de la magnitud del vector de separación $\|h\|$. 

Si trabajamos con coordenadas geográficas (Latitud/Longitud, EPSG:4326), las unidades son grados angulares. La distancia euclidiana entre dos puntos en grados no representa la distancia física real sobre la superficie terrestre, y esta distorsión varía según la latitud. Para garantizar que la distancia $\|h\|$ sea métricamente válida, debemos transformar los datos a un Sistema de Coordenadas Proyectadas (PCS). 

Para el área de estudio de Bogotá D.C., el sistema oficial y más adecuado es el *UTM (Universal Transverse Mercator) Zona 18 Norte*, con datum WGS84 (código *EPSG: 32618*). En este sistema, las unidades son metros, lo que garantiza que el "rango" o alcance espacial del modelo teórico tenga una interpretación física real.


```{r fig.height=5, fig.width=11, message=FALSE, warning=FALSE, include=FALSE}
...
El estimador clásico de Matheron es sensible a valores atípicos. Dado que la concentración de $\text{PM}_{10}$ puede presentar episodios de alta contaminación localizados o errores instrumentales (como se evidenció en la asimetría positiva de la exploración), se implementa también el estimador robusto de Cressie y Hawkins ($\text{modulus}$). Este estimador atenúa el impacto desproporcionado de los incrementos extremos al elevar a la potencia $1/2$ antes de promediar.
...
Sse calcula el semivariograma restringiendo los pares de puntos a ángulos específicos ($0^\circ, 45^\circ, 90^\circ, 135^\circ$) con una tolerancia angular de $\pm 22.5^\circ$. Esto permite evaluar si la topografía de Bogotá (los Cerros Orientales) o los patrones de viento generan una anisotropía geométrica, donde el alcance espacial varía según la dirección.
...
Al observar los gráficos se evidencia que el semivariograma direccional a 90° (Este-Oeste) alcanza la meseta más rápidamente, mientras que el semivariograma a 0° (Norte-Sur) muestra un alcance más largo. Esto sugiere una anisotropía geométrica, probablemente influenciada por la topografía de Bogotá y los patrones de viento predominantes.


## Estimación teórica del semivariograma

Una vez obtenido el semivariograma empírico de los residuales, se ajusta un modelo teórico válido (condicionalmente definido negativo). Los parámetros fundamentales a estimar son:

*   **Efecto Pepita ($c_0$):** Discontinuidad en el origen, atribuible a error de medición o microestructura (variabilidad a escalas menores a la distancia mínima entre estaciones).
*   **Silla Parcial ($c$):** Varianza explicada puramente por la estructura de autocorrelación espacial.
*   **Silla Total ($C(0) = c_0 + c$):** Varianza total del proceso estocástico $\hat{Z}(s)$.
*   **Alcance o Rango ($a$):** Distancia a partir de la cual las observaciones dejan de estar espacialmente correlacionadas.
...
# Predicción con krigin universal

```{r fig.height=5, fig.width=10}

# ==============================================================================
# 5. PREDICCIÓN ESPACIAL: KRIGING UNIVERSAL
# ==============================================================================
# Teoría: Bohórquez (2025), Cap. 4. El Kriging Universal modela la tendencia 
# determinística μ(s) y la autocorrelación espacial de los residuales Z(s) 
# simultáneamente, garantizando insesgamiento y varianza mínima.
...
# ------------------------------------------------------------------------------
# 1. PREPARACIÓN DE DATOS EN SISTEMA PROYECTADO (UTM 18N - Metros)
# ------------------------------------------------------------------------------
# Aseguramos que todo el flujo de predicción trabaje en metros (EPSG:32618)
datos_utm <- st_as_sf(
  datos_corte, 
  coords = c("Longitud", "Latitud"), 
  crs = 4326
) %>%
  st_transform(crs = 32618) %>% # Transformamos a UTM 18N
  mutate(
    x = st_coordinates(.)[,1],
    y = st_coordinates(.)[,2]
  )

# Transformamos el shapefile de Bogotá al mismo CRS (Metros)
bogota_utm <- st_transform(bogota, crs = 32618)

# ------------------------------------------------------------------------------
# 2. AJUSTE DEL MODELO TEÓRICO (Exclusivo con gstat)
# ------------------------------------------------------------------------------
# gstat calcula el variograma empírico SOBRE LOS RESIDUALES de la tendencia ~x
# internamente, gracias a la fórmula.
v_emp_gstat <- variogram(PM10 ~ x, data = datos_utm, cutoff = 15000)

# Ajuste teórico (usaremos un modelo Esférico como ejemplo, ajusta según tu AIC)
# Inicializamos con la varianza muestral y un rango visual razonable en metros
modelo_kriging <- fit.variogram(
  v_emp_gstat, 
  vgm(psill = var(datos_utm$PM10), model = "Sph", range = 5000, nugget = 10)
)

# ------------------------------------------------------------------------------
# 3. CREACIÓN DE GRILLA DE PREDICCIÓN (En Metros)
# ------------------------------------------------------------------------------
# Usamos st_make_grid (estándar moderno de sf) para evitar errores de bbox
grilla_utm <- st_make_grid(
  bogota_utm,             # Usamos el bbox de Bogotá en UTM
  n = c(150, 150),        # Resolución de la grilla (150x150 pixeles)
  what = "centers"
) %>% 
  st_sf() %>%
  mutate(
...
# ------------------------------------------------------------------------------
# 4. EJECUCIÓN DEL KRIGING UNIVERSAL
# ------------------------------------------------------------------------------
# Al pasar la fórmula PM10 ~ x, gstat ejecuta Kriging Universal automáticamente
kriging_result <- krige(
  formula = PM10 ~ x,         # Tendencia de primer orden en X
  locations = datos_utm,      # Datos observados en UTM
  newdata = grilla_utm,       # Grilla de predicción en UTM
  model = modelo_kriging      # Modelo teórico ajustado
)

# ------------------------------------------------------------------------------
# 5. ENMASCARAMIENTO (MASCARA DE AGUA / LÍMITES DE BOGOTÁ)
# ------------------------------------------------------------------------------
# Recortamos la grilla para que solo muestre la forma de Bogotá
kriging_masked <- st_intersection(kriging_result, bogota_utm)

# ------------------------------------------------------------------------------
# 6. VISUALIZACIÓN PROFESIONAL CON ggplot2
# ------------------------------------------------------------------------------
# Mapa 1: Predicción Espacial (PM10)
p1 <- ggplot() +
  geom_sf(data = kriging_masked, aes(color = var1.pred), size = 0.4) +
  geom_sf(data = bogota_utm, fill = NA, color = "black", linewidth = 0.5) +
  geom_sf(data = datos_utm, color = "red", size = 1.5, shape = 4) + # Estaciones
  scale_color_viridis_c(name = expression(PM[10]~(mu*g/m^3)), option = "magma") +
  labs(
    title = "Predicción Espacial: Kriging Universal",
    subtitle = "Tendencia modelada: μ(s) ~ Longitud (x)",
    x = "Coordenada Este (m)", y = "Coordenada Norte (m)"
  ) +
  theme_minimal() +
  theme(
...
# Mapa 2: Varianza del Error de Kriging (Incertidumbre)
p2 <- ggplot() +
  geom_sf(data = kriging_masked, aes(color = var1.var), size = 0.4) +
  geom_sf(data = bogota_utm, fill = NA, color = "black", linewidth = 0.5) +
  geom_sf(data = datos_utm, color = "red", size = 1.5, shape = 4) +
  scale_color_viridis_c(name = "Varianza\nde Kriging", option = "plasma", direction = -1) +
  labs(
    title = "Incertidumbre de la Predicción",
    subtitle = "Varianza del error (mínima en estaciones observadas)",
    x = "Coordenada Este (m)", y = "Coordenada Norte (m)"
  ) +
  theme_minimal() +
  theme(
...
# Imprimir ambos mapas lado a lado
p1 + p2
```

En el mapa de Predicción Espacial se aprecia una clara gradación espacial de $\text{PM}_{10}$ orientada de oriente a occidente, congruente con la tendencia de primer orden modelada en función de la coordenada $x$. Las concentraciones más elevadas ($> 80\ \mu\text{g/m}^3$, tonos claros) se localizan hacia la franja occidental del dominio, mientras que los Cerros Orientales exhiben los niveles más bajos de contaminación.Por su parte, el mapa de Incertidumbre de la Predicción (Varianza de Kriging) refleja fielmente la propiedad teórica del Kriging Universal: la varianza de estimación es mínima (puntos amarillos $\approx 0 - 100$) justamente sobre las ubicaciones directas de las estaciones de monitoreo, donde el predictor honra los datos observados. A medida que los puntos de interpolación se alejan de la red de monitoreo, la incertidumbre se incrementa gradualmente (tonos morados oscuros $> 300$), alcanzando sus valores máximos en los extremos geográficos y bordes del área de estudio con menor cobertura muestral.
...
coord <- st_coordinates(datos_modelo)

D <- as.matrix(dist(coord))

z <- datos_modelo$resid_lineal
z <- z - mean(z)
nugget_fijo <- modelo_teorico_universal$psill[1]

# Esférico
gamma_sph <- function(h, nugget, sill, range){
...
#Gaussiano
gamma_gau <- function(h, nugget, sill, range){
...
gamma_mix <- function(h,
  ...
  w[1]*gamma_sph(h,nugget,sill,range)+
  w[2]*gamma_exp(h,nugget,sill,range)+
  w[3]*gamma_gau(h,nugget,sill,range)

}

gamma_model <- function(h,
                        nugget,
                        sill,
                        range,
                        model){
  ...
  switch(
    ...
    "Sph" = gamma_sph(h, nugget, sill, range),

    "Exp" = gamma_exp(h, nugget, sill, range),

    "Gau" = gamma_gau(h, nugget, sill, range),

    "Mix" = gamma_mix(h, nugget, sill, range)
...
compLik <- function(par, D, z, nugget, model){
...
Se implementó la estimación de los parámetros del semivariograma mediante el método de verosimilitud compuesta utilizando la función *optim()* de R. Se ajustaron modelos esférico, exponencial y gaussiano, además de una combinación lineal de estos con coeficientes positivos estimados mediante optimización. La comparación de los valores de la función objetivo mostró que el modelo gaussiano presentó el mejor ajuste (755.7683). La combinación lineal no mejoró el valor de la función de verosimilitud compuesta y asignó todo el peso al componente gaussiano, indicando que este modelo describe adecuadamente la estructura espacial de los datos.
...
1. **¿Para un $\|h\| = 10$ con $\phi = 0^\circ$, esto es, en dirección horizontal, para $s = (9564, 999800)$; $Z(s) = ?$ y $Z(s + h) = ?$**
    ...
    **R/**
    ...
    Para la dirección horizontal ($\phi = 0^\circ$) con una magnitud de rezago $\|h\| = 10$, el vector de desplazamiento es $h = (10, 0)$. Esto significa que nos movemos a lo largo del eje X (Este).
    
    *   **Punto inicial $s$:** Evaluando la Tabla en las coordenadas $s = (9564, 999800)$ es $Z(s) = 390$
    *   **Punto desplazado $s+h$:** Aplicando el vector de rezago, la ubicación es $s+h = (9564 + 10, 999800 + 0) = (9574, 999800)$. Al inspeccionar la matriz de datos para esta coordenada, observamos que no existe una medición registrada:
        $$Z(s+h) = \text{NA} \quad (\text{Dato faltante})$$
...
2. **¿Para $\phi = 45^\circ$, es posible encontrar pares de datos? En caso afirmativo, encuentre $\|h\|$ y $n(\|h\|)$.**
    ...
    * **Cálculo de $\|h\|$:** La distancia euclidiana entre estos puntos diagonales adyacentes se calcula mediante el teorema de Pitágoras:
    $$\|h\| = \sqrt{10^2 + 10^2} = \sqrt{200} = 10\sqrt{2} \approx 14.14$$
    * **Cálculo de $n(\|h\|)$:** Para determinar el número de pares válidos $n(\|h\|)$, se cuentan todas las combinaciones de puntos $(s_i, s_j)$ en la grilla que estén separados exactamente por el vector $h = (10, 10)$ y que ambos posean datos válidos (no sean `NA`). 
    
    
    ```{r ej-1-2, message=FALSE, warning=FALSE}
# Construcción del data.frame espacial (formato largo)
grid <- expand.grid(X = coords_x, Y = coords_y)
grid$Z <- as.vector(t(matriz_alturas)) # t() para respetar el orden de expand.grid

# Filtrado de pares con vector de rezago h = (10, 10)
pares_45 <- merge(grid, grid, by = NULL, suffixes = c("_i", "_j"))
pares_45 <- subset(pares_45, (X_j - X_i == 10) & (Y_j - Y_i == 10))

# Exclusión de pares donde al menos uno de los dos puntos sea NA
pares_validos <- subset(pares_45, !is.na(Z_i) & !is.na(Z_j))

# Resultado
n_h <- nrow(pares_validos)
cat("Distancia ||h||:", round(10 * sqrt(2), 4), "\n")
cat("Numero de pares validos n(||h||):", n_h, "\n\n")

# Desglose de los 12 pares válidos encontrados
cat("Desglose de los pares (s_i -> s_j):\n")
for(i in 1:nrow(pares_validos)) {
  cat(sprintf("(%d, %d) -> (%d, %d)  | Z: %d -> %d\n", 
              pares_validos$X_i[i], pares_validos$Y_i[i], 
              pares_validos$X_j[i], pares_validos$Y_j[i], 
              pares_validos$Z_i[i], pares_validos$Z_j[i]))
}
    ```
...
3. **La media es constante e igual a 460. Si la covarianza espacial es un modelo esférico sin pepita y con silla 25000 y rango 15, ¿Cuál es el valor del coeficiente de autocorrelación para $\|h\| = 30$ con $\phi = 90^\circ$?**
    ...
    **R/**
    ...
    Para $\|h\| = 30$ con $\phi = 90^\circ$, la autocorrelación es ${\rho(30) = 0}$ (ya que la distancia excede el rango $a = 15$).
...
1. **La estacionariedad intrínseca implica la estacionariedad de segundo orden. Ilustre con ejemplos.**
    ...
    **R/**
    ...
    Por otro lado, la estacionariedad de segundo orden (o débil) exige que 
    
    1. La varianza marginal $Var[Z(s)] = C(0) < \infty$ 
    2. La covarianza $Cov[Z(s), Z(s+h)] = C(h)$ existan y sean finitas para todo $s$.
...
2. **La estacionariedad de segundo orden implica la estacionariedad intrínseca. Ilustre con ejemplos.**
    ...
    **R/**
    ...
    Suponga que el proceso $Z(s)$ es estacionario de segundo orden. Esto implica que:
    
    1. $E[Z(s)] = \mu$ (media constante para todo $s$).
    2. $Cov[Z(s), Z(s+h)] = C(h)$ (la covarianza depende solo del incremento $h$).
    3. $Var[Z(s)] = C(0) < \infty$.
    
    Con esto las dos condiciones de la estacionariedad intrínseca para los incrementos $Z(s+h) - Z(s)$ se cumplen automáticamente:
    ...
    *(Es constante).*
    ...
    Puesto que $C(0)$ y $C(h)$ dependen únicamente del vector $h$ y no de la ubicación $s$, el semivariograma $\gamma(h)$ también depende solo de $h$. 
    
    Por lo tanto, todo proceso estacionario de segundo orden es intrínsecamente estacionario.
...
1. **¿Cómo es el comportamiento de la elevación del terreno en cada una de las coordenadas geográficas? ¿Existe tendencia?**
    ...
    * **Respuesta:** *(Analizar según la Figura 1.4 de tu documento)*. Si la elevación aumenta o disminuye sistemáticamente a medida que nos desplazamos en la dirección Este (X) o Norte (Y), existe una **tendencia espacial de primer u orden superior**.
    Para determinar si existe tendencia, se analizar si la media del proceso $E[Z(s)]$ varía con las coordenadas espaciales.
    
    * ***En el eje X (Este):** El gráfico de dispersión muestra una tendencia positiva directa. A medida que la coordenada $X$ aumenta, la elevación del terreno incrementa de manera lineal.
    * **En el eje Y (Norte):** El gráfico de dispersión evidencia una tendencia negativa; a medida que la coordenada $Y$ aumenta (se desplaza hacia el Norte), la elevación del terreno disminuye.

    Sí existe una tendencia espacial de primer y segundo orden, pues la media de la elevación no es constante, sino que depende de la ubicación, luego se necesita modelar $\mu(s)$ antes de aplicar geoestadística sobre los residuales.

    
    Sí existe una fuerte tendencia espacial de primer y segundo orden**, por lo que la media de la elevación depende de la ubicación geográfica.

2. **Encuentre la correlación de Pearson entre la elevación y cada una de las coordenadas espaciales. Interprete.**

    **R/**
    
    La correlación de Pearson ($r$) mide la fuerza de la relación lineal entre dos variables. En el contexto espacial, una correlación alta con las coordenadas confirma la presencia de una tendencia.

    En el resumen del modelo nos proporciona la correlación múltiple ($R$) y el coeficiente de determinación ($R^2$):

    *   **Coeficiente de Correlación Múltiple ($R$):** $\mathbf{0.899}$
    *   **Coeficiente de Determinación ($R^2$):** $\mathbf{0.8091}$

    Luego existe una correlación lineal muy alta ($\approx 0.90$) entre el conjunto de coordenadas espaciales y la elevación. El $80.91\%$ de la variabilidad total de la elevación del terreno se explica únicamente por la posición geográfica ($X$, $Y$ y su interacción). Esto justifica estadísticamente el uso de un modelo de regresión espacial para remover la tendencia.
...
3. **¿Considera que es necesario incluir el análisis de la interacción entre las coordenadas geográficas este y norte en el modelo de regresión? Justifique.**
    ...
    *Sí, es totalmente necesario.* 
    ...
    La inclusión del término de interacción ($X \cdot Y$) permite capturar superficies de tendencia que no están estrictamente alineadas con los ejes, es decir, superficies con curvatura diagonal.
    
    En la tabla de coeficientes, el término de interacción `Este*Norte` presenta un coeficiente de $\beta_3 = -0.00015$ con un **valor p de $3.23 \times 10^{-70}$ (prácticamente $0 < 0.05$). Esto demuestra que el efecto de la coordenada $X$ sobre la elevación cambia dependiendo del valor de la coordenada $Y$. Omitir este término resultaría en un modelo mal especificado (sesgo de variable omitida), lo que contaminaría los residuales y, por ende, la estimación posterior del semivariograma.
...
4. **¿Considera usted que el proceso espacial posee media estacionaria? Justifique.**
    ...
    *No, el proceso NO posee media estacionaria.*

    Un proceso espacial cumple con estacionariedad de la media (o estacionariedad débil) si y solo si $E[Z(s)] = \mu$ (una constante finita para todo $s \in D_s$). 
    
    En este caso, la elevación varía sistemáticamente en función de la ubicación geográfica, como lo demuestra el $R^2 = 80.91\%$ y la significancia de los coeficientes espaciales. Por lo tanto, la media es una función de las coordenadas:
    $$E[Z(s)] = \mu(s) = \beta_0 + \beta_1 X + \beta_2 Y + \beta_3 XY$$
    
    Para poder aplicar herramientas geoestadísticas (como el semivariograma y el kriging), debemos asumir estacionariedad intrínseca o de segundo orden sobre los residuales $Z(s) = Y(s) - \mu(s)$, no sobre la variable original.
...
5. **Interprete los colores en el gráfico del panel superior izquierdo. ¿Es posible analizar tendencia en este gráfico?**
    ...
    **R/**
    ...
    La escala de colores representa la magnitud de la elevación, dividiendo las muestras en cuartiles. Los colores oscuros/morados corresponden a las elevaciones más bajas, mientras que los colores claros/verdes representan las elevaciones más altas.
    
    Sí es posible analizar tendencia visualmente. Se observa una clara separación y gradiente espacial de los colores: los puntos morados (baja elevación) se concentran en el sector inferior derecho (mayor $X$, menor $Y$), mientras que los verdes/claros se dispersan hacia el norte/occidente. Esto confirma lo detectado por el modelo de regresión.
...
6. **Escriba formalmente el modelo de regresión estimado. Si el valor de un residual en un lugar $s_0$ es 10, ¿cuál es el valor de la variable de interés en ese lugar?**
    ...
    **R/**
     ...
     **Modelo Formal:**
     Basado en la tabla de coeficientes, el modelo de regresión que describe la tendencia es:
    ...
    Para calcular la variable de interés $Z(s_0)$ se hace uso de la relación entre el valor real $Z(s_0)$, la tendencia predicha $\hat{Z}(s_0)$ y el residual $e(s_0)$
    
    
    $$Z(s_0) = \hat{Z}(s_0) + e(s_0)$$
    
    Si en la ubicación $s_0 = (X_0, Y_0)$ el residual es $e(s_0) = 10$:
    
    $$Z(s_0) = \left( 734.7 + 0.25 \cdot X_0 - 0.21 \cdot Y_0 - 0.00015 \cdot X_0 Y_0 \right) + 10$$

    El valor de la elevación en el lugar $s_0$ es 10 metros superior a la altura estimada por la superficie de tendencia en ese punto.
...
1. **Encuentre la matriz de correlación de la variable y sus coordenadas geográficas. Analice.**
    ...
    **R/**
    ...
    ```
    Ninguna de las dos coordenadas geográficas presenta una fuerte asociación lineal con la sensación térmica ($|r| < 0.15$). Esto sugiere que no es necesario incluir un modelo de tendencia polinomial complejo (Kriging Universal) y que podemos asumir, en primera instancia, una media constante.
...
2. **Realice gráficos de dispersión de la variable contra cada una de las coordenadas geográficas. Analice.**
    ...
    **R/**
    ...
    ```{r paso-2-dispersion-coords, fig.width=10, fig.height=4}
    ...
    p1 + p2
    ```
    Los gráficos confirman el supuesto de estacionariedad de primer orden ($E[Z(s)] = \mu$). La media de la sensación térmica no varía sistemáticamente en función de la ubicación geográfica. El proceso es adecuado para aplicar Kriging Ordinario.
...
3. **Realice gráficos de dispersión de la media (mediana) de cada columna con respecto a su coordenada horizontal.**
    ...
    **R/**
    ...
    ```{r paso-3-media-columna, fig.height=3, fig.width=10}
    ...
    La media y la mediana fluctúan de manera estable alrededor de $20.25^\circ\text{C}$. Dado que la media no es resistente a valores atípicos (como se observa en `Este = 8`), en etapas posteriores se preferirá usar estimadores resistentes de semivarianza (Cressie & Hawkins o Mediana) o el pulimiento de medianas.
...
4. **Realice gráficos de dispersión de la media (mediana) de cada fila con respecto a su coordenada vertical.**
    ...
    **R/**
    ...
    ```{r paso-4-media-fila, fig.height=3, fig.width=10}
    ...
    El comportamiento oscilatorio sin una dirección monótona clara refuerza la hipótesis de estacionariedad de primer orden.
...
5. **Realice gráficos de dispersión de la varianza de cada columna con respecto a su coordenada horizontal.**
    ...
    **R/**
    ...
    ```{r paso-5-varianza-columna, fig.height=3.5, fig.width=10}
...
6. **Realice gráficos de algunos dispersogramas. Encuentre el coeficiente de autocorrelación para cada caso. Analice.**
    ...
    **R/**
    ...
    ```{r paso-6-dispersogramas, fig.width=10, fig.height=4}
    ...
    p_h + p_v
    ```
    Se observa una correlación espacial positiva tanto en la dirección horizontal como en la vertical, lo que confirma la presencia de dependencia espacial que debe ser modelada.
...
7. **Realice gráficos de dispersión de la varianza de cada fila con respecto a su coordenada vertical.**
    ...
    **R/**
    ...
    ```{r paso-7-varianza-fila, fig.height=3.5, fig.width=10}
...
8. **Realice al menos un gráfico de medias móviles que no sea ni por fila ni por columna.**
    ...
    **R/**
    ...
    ```{r paso-8-media-movil-diagonal, fig.height=3.5, fig.width=10}
...
datos_validos_sf <- st_as_sf(datos_validos, coords = c("Este", "Norte"))
    ...
    Dado que los modelos de regresión no logran capturar una tendencia determinística clara (y el AIC no muestra una mejora sustancial que justifique un Kriging Universal), asumiremos media constante. Para el modelamiento del semivariograma, utilizaremos los datos centrados respecto a su media global o aplicaremos el pulimiento de medianas (paso 17) para mayor robustez ante los valores atípicos identificados.
...
15. **¿Tienen un comportamiento similar los semivariogramas en todas las direcciones? ¿En todas las direcciones el semivariograma empírico sugiere ajustar un modelo acotado? Esto es, ¿se observa que existen silla y rango? o por el contrario ¿la varianza no parece estabilizarse?**
    ...
    **R/**
    ...
    Al comparar los gráficos de los pasos 10 al 14, se observa que los semivariogramas presentan un comportamiento similar en todas las direcciones (alcances y pendientes comparables), lo que permite asumir Isotropía. Además, la varianza tiende a estabilizarse (meseta), lo que sugiere que el proceso es estacionario de segundo orden y es adecuado ajustar un modelo acotado (como el Esférico o Exponencial).
...
16. **Ajuste un modelo de semivariograma teórico omnidireccional. Según lo encontrado en el análisis a diferentes ángulos, determine si es necesario aplicar corrección por anisotropía.**
    ...
    **R/**
    ...
    ```
    Dado que no se evidenciaron diferencias significativas en los rangos o pendientes según la dirección (Paso 15), no es necesario aplicar corrección por anisotropía geométrica. El modelo esférico omnidireccional es suficiente para describir la dependencia espacial.
...
18. **Realice la validación cruzada (al menos para 10 lugares observados) y calcule:**
    ...
    **R/**
    ...
    ```
    La validación cruzada muestra que los errores tienen una media cercana a cero y el coeficiente de correlación de Pearson entre los valores observados y predichos es alto, lo que valida la capacidad predictiva del modelo de Kriging Ordinario aplicado sobre los residuales del pulimiento de medianas.
...
### Realice una tabla comparativa entre el método de ecuaciones de estimación generalizada y el método de $CL$. 

Encuentre similitudes y diferencias.

**R/**

Tanto el método de Mínimos Cuadrados Generalizados (MCG) como el método de Pseudoverosimilitud Compuesta (CL, *Composite Likelihood*) son alternativas al método de Máxima Verosimilitud (ML) para estimar los parámetros $\Theta$ de un modelo de semivarianza o covarianza espacial. Sin embargo, abordan el problema desde perspectivas matemáticas y computacionales distintas.

A continuación, se presenta la tabla comparativa basada en los la Sección 3.5 y la Nota 5 (Sección 3.6) del libro:

| Característica | MCG (Ecuaciones de Estimación Generalizada) | CL (Pseudoverosimilitud Compuesta) |
| :--- | :--- | :--- |
| **Función Objetivo** | Minimiza la distancia cuadrática entre el semivariograma empírico y el teórico, ponderada por la inversa de la matriz de covarianza del estimador empírico: $(\hat{\gamma} - \gamma(\Theta))' V^{-1} (\hat{\gamma} - \gamma(\Theta))$. | Maximiza la suma de las funciones de log-verosimilitud marginales de los incrementos $U_{ij} = Z(s_i) - Z(s_j)$: $CL(\Theta) = \sum l(U_{ij}, \Theta)$. |
| **Supuesto Distribucional** | Requiere conocer o estimar la matriz de covarianza $V$ de los incrementos al cuadrado, lo cual implica asumir normalidad bivariada para derivar la varianza del estimador empírico. | Solo requiere conocer las distribuciones marginales univariadas (o bivariadas) de los incrementos. Relaja el supuesto de normalidad multivariada conjunta. |
| **Complejidad Computacional** | Alta y problemática. La matriz de ponderación $V$ depende de $\Theta$, y para estimar $V$ se requiere conocer la matriz de covarianza espacial $\Sigma$, creando una dependencia circular. | Baja y eficiente. El proceso de optimización solo involucra matrices de dimensión $p \times p$ (donde $p$ es el número de parámetros), evitando la inversión de matrices $N \times N$. |
| **Uso de los Datos** | Trabaja sobre los pares de distancia y semivarianza ya agrupados en clases (el semivariograma empírico binneado). | Trabaja directamente con todas las realizaciones posibles de los incrementos espaciales (o un subconjunto filtrado por umbrales de alcance). |
| **Eficiencia Estadística** | Menor eficiencia. La dificultad para estimar $V$ de manera consistente en la práctica limita su rendimiento. | Mayor eficiencia que MCG. Aunque es menos eficiente que ML, su eficiencia aumenta notablemente al usar ponderaciones dicotómicas basadas en los alcances espaciales. |

En la práctica, el método CL es preferido sobre MCG, intentar usar MCG complicaría el procedimiento, pues la estimación de la matriz $\Sigma$ implicaría la estimación previa de $V$, haciendo que el método no sea práctico.

```{r ej-3-1-comparacion, echo=FALSE, message=FALSE, warning=FALSE}
# Implementación conceptual en R de las funciones objetivo

# 1. Función objetivo de MCG (Mínimos Cuadrados Generalizados)
# Requiere el vector de semivarianza empírica (gamma_hat), el vector teórico (gamma_teo)
# y la matriz de ponderación V (inversa de la varianza del estimador empírico).
mcg_objetivo <- function(gamma_hat, gamma_teo, V_inv) {
  t(gamma_hat - gamma_teo) %*% V_inv %*% (gamma_hat - gamma_teo)
}

# 2. Función objetivo de CL (Pseudoverosimilitud Compuesta)
# Suma de las log-verosimilitudes marginales de los incrementos U_ij ~ N(0, 2*gamma(h_ij))
cl_objetivo <- function(incrementos, distancias, theta) {
  # theta contiene los parámetros del modelo de semivarianza
  # gamma_teo es el vector de semivarianzas teóricas evaluadas en las distancias
  gamma_teo <- modelo_semivarianza(distancias, theta) 
  
  # Log-verosimilitud normal marginal para cada incremento
  log_verosimilitudes <- dnorm(incrementos, mean = 0, sd = sqrt(2 * gamma_teo), log = TRUE)
  
  return(sum(log_verosimilitudes))
}

# Nota: Mientras MCG requiere construir y invertir V (dependiente de Sigma), 
# CL solo evalúa la densidad normal marginal para cada par, escalando linealmente 
# con el número de pares, no con el cubo o la cuarta potencia de N.
```

### Suponga que se tiene un modelo de covarianza y se requiere elegir entre dos vectores de parámetros, cuál de ellos es más verosímil con base en el vector de observaciones.

Bajo el supuesto de normalidad, describa en general el procedimiento e ilustre para al menos un caso particular.

**R/**


Bajo el supuesto de normalidad multivariada, el vector de observaciones $Y = (Y(s_1), \dots, Y(s_n))'$ sigue una distribución $Y \sim N_n(X\beta, \Sigma(\Theta))$, donde $\boldsymbol\Sigma(\Theta)$ es la matriz de covarianza construida a partir de una función definida positiva $C(h; \Theta)$ con parámetros $\Theta = (\sigma^2, a, \tau^2)'$ (silla, rango práctico y efecto pepita).


Para comparar dos vectores de parámetros candidatos , $\Theta_1$ y $\Theta_2$, se utiliza el criterio de Máxima Verosimilitud. El procedimiento es el siguiente:

1.  Se asume el proceso centrado $Z = Y - \mu$.
2.  Se evalúa la función del negativo de la log-verosimilitud para cada vector. La expresión a minimizar es:
    $$L({\Theta}) = \frac{n}{2} \ln(2\pi) + \frac{1}{2} \ln|\boldsymbol{\Sigma}({\Theta})| + \frac{1}{2} Z' \boldsymbol{\Sigma}^{-1}({\Theta}) Z$$
3.  Se evalúan  $L(\Theta_1)$ y $L(\Theta_2)$.
4.  Se selecciona el vector ${\Theta}^* = \arg\min_{{\Theta}} L(\boldsymbol{\Theta})$. 
El vector óptimo es aquel que logra el mejor balance entre la penalización por dispersión ($\ln|\boldsymbol{\Sigma}|$) y el ajuste a la variabilidad/correlación observada en los datos ($Z'\boldsymbol{\Sigma}^{-1}Z$).


Consideremos $n = 2$ observaciones $z = (3, 4)'$ separadas por una distancia $h = 5$. Se asume un modelo exponencial $C(h; \boldsymbol{\Theta}) = \sigma^2 \exp(-h/a)$. Evaluamos:

* ${\Theta}_1 = (\sigma^2 = 10, a = 2)$
* ${\Theta}_2 = (\sigma^2 = 20, a = 10)$

```{r ej-3-2-verosimilitud, echo=TRUE}
# --- Función Optimizada de Negativo de Log-Verosimilitud ---
neg_log_likelihood <- function(z, sigma2, a, dist_matrix) {
  n <- length(z)
  Sigma <- sigma2 * exp(-dist_matrix / a)
  
  # Cálculo numéricamente estable de log-determinante y forma cuadrática
  log_det_Sigma <- as.numeric(determinant(Sigma, logarithm = TRUE)$modulus)
  quad_form <- as.numeric(t(z) %*% solve(Sigma, z))
  
  L <- 0.5 * (n * log(2 * pi) + log_det_Sigma + quad_form)
  return(L)
}

# --- Configuración del Caso ---
z <- c(3, 4)
dist_matrix <- matrix(c(0, 5, 5, 0), nrow = 2)

Theta_1 <- c(sigma2 = 10, a = 2)
Theta_2 <- c(sigma2 = 25, a = 10)

L_1 <- neg_log_likelihood(z, Theta_1["sigma2"], Theta_1["a"], dist_matrix)
L_2 <- neg_log_likelihood(z, Theta_2["sigma2"], Theta_2["a"], dist_matrix)

cat(sprintf("NLL para Theta_1 (10, 2)  : %.4f\n", L_1))
cat(sprintf("NLL para Theta_2 (25, 10) : %.4f\n", L_2))
```


En este ejemplo, la verosimilitud selecciona ${\Theta}_2$ debido a que $L({\Theta}_2) = 5.1578 < L({\Theta}_1) = 5.2964$. 

Aunque $\boldsymbol{\Theta}_2$ impone una varianza mayor ($\sigma^2 = 25$), su mayor rango espacial ($a = 10$) captura adecuadamente la covarianza entre los puntos ($C(5) = 12.13$), lo que reduce drásticamente el valor de la forma cuadrática $Z'\boldsymbol{\Sigma}^{-1}Z$ y compensa la penalización del determinante.


### Plantee para cada método y para cada modelo de covarianza o semivarianza la expresión a minimizar:

Para plantear las expresiones a minimizar, asumiremos un modelo de semivarianza teórico $\gamma(h; \Theta)$, donde $\Theta$ es el vector de parámetros a estimar. Sea $\hat{\gamma}(h_k)$ el estimador empírico para el rezago $h_k$, con $K$ rezagos totales, y $|N(h_k)|$ la cantidad de pares de datos en dicho rezago.

1. **Mínimos cuadrados ordinarios (MCO),**

    **R/**
    
    El método MCO trata la estimación del semivariograma como un problema de regresión no lineal estándar, donde la "variable respuesta" son los valores empíricos $\hat{\gamma}(h_k)$.
$$Q_{MCO}(\Theta) = \sum_{k=1}^K \left( \hat{\gamma}(h_k) - \gamma(h_k; \Theta) \right)^2$$
    
2. **Mínimos cuadrados ponderados (MCP) considerando ambas ponderaciones:**

    **R/**
    
    Dado que la varianza de los estimadores empíricos cambia drásticamente con la distancia (y con la cantidad de pares $|N(h_k)|$), el MCP introduce una matriz de ponderación $W$.
    
    a. **La varianza del estimador empírico del semivariograma**
        Bajo el supuesto de normalidad marginal, la varianza de los incrementos al cuadrado permite construir la matriz diagonal $W(\Theta) \simeq \text{Diag}\left( \frac{2\gamma(h_k; \Theta)^2}{|N(h_k)|} \right)$. La expresión a minimizar es:
        $$Q_{MCP\_var}(\Theta) = \left( \hat{\boldsymbol{\gamma}} - \boldsymbol{\gamma}(\Theta) \right)^\top W^{-1}(\Theta) \left( \hat{\boldsymbol{\gamma}} - \boldsymbol{\gamma}(\Theta) \right)$$
        Este método es iterativo porque la matriz de pesos $W$ depende del parámetro $\Theta$ que se está estimando.

    b. **$N(h)$, número de pares de datos para cada rezago espacial considerado $h$**
        Para evitar la iteración de la matriz de pesos, se puede usar una matriz constante basada en la inversa de la cantidad de pares: $W = \text{Diag}\left( \frac{1}{|N(h_k)|} \right)$. 
        $$Q_{MCP\_npairs}(\Theta) = \sum_{k=1}^K |N(h_k)| \left( \hat{\gamma}(h_k) - \gamma(h_k; \Theta) \right)^2$$

3. **ML, con el supuesto de normalidad multivariada conjunta.**

    **R/**
    
    sumiendo normalidad multivariada conjunta $Y \sim N_n(X\beta, \Sigma(\Theta))$, se minimiza el negativo de la log-verosimilitud. Aquí $\Sigma(\Theta)$ es la matriz de covarianza $n \times n$ construida evaluando el modelo teórico entre todos los pares de observaciones.
    $$L_{ML}(\beta, \Theta) = \frac{n}{2}\ln(2\pi) + \frac{1}{2}\ln|\Sigma(\Theta)| + \frac{1}{2}(Y - X\beta)^\top \Sigma^{-1}(\Theta) (Y - X\beta)$$
    
4. **REML, con el supuesto de normalidad multivariada conjunta.**

    **R/**
    
    Para reducir el sesgo en la estimación de $\Theta$ al no estimar la tendencia $\beta$ simultáneamente, se maximiza la verosimilitud de los incrementos $A^\top Y$ (donde $A$ es una matriz de contraste tal que $A^\top X = 0$). Se minimiza:
    $$L_{REML}(\Theta) = \frac{n-p}{2}\ln(2\pi) + \frac{1}{2}\ln|A^\top \Sigma(\Theta) A| + \frac{1}{2}Y^\top A (A^\top \Sigma(\Theta) A)^{-1} A^\top Y$$
    
5. **Pseudoverosimilitud ($CL$), con el supuesto de normalidad multivariada marginal.**

    **R/**
    
    Relaja el supuesto de normalidad conjunta y utiliza las distribuciones marginales de los incrementos $U_{ij} = Y(s_i) - Y(s_j) \sim N(0, 2\gamma(s_i - s_j; \Theta))$. Se minimiza el negativo de la log-verosimilitud compuesta:
    $$-CL(\Theta) = \sum_{i=1}^{n-1} \sum_{j=i+1}^n \left[ \frac{1}{2}\ln\left(4\pi\gamma(s_i - s_j; \Theta)\right) + \frac{U_{ij}^2}{4\gamma(s_i - s_j; \Theta)} \right]$$
...
6. **Diseñe el código para que estos procesos de optimización, funcionen desde la tabla de los datos originales. Puede usar, por ejemplo, la función `optim` de R cran project. Compare los resultados obtenidos con su código con los obtenidos por las funciones de `geoR`, `variofit`, `likfit` o de `gstat`, `fit.variogram`.**
    ...
    **R/**
    ...
    A continuación, se presenta un script que simula datos, define las funciones objetivo para MCO (basado en el variograma empírico) y ML (basado en la matriz de covarianza), y utiliza `optim()` para encontrar $\Theta$. Luego, se comparan los resultados con `gstat` y `geoR`.

    ```{r estimacion-parametros-completa, message=FALSE, warning=FALSE}
    library(gstat)
    library(geoR)
    library(MASS)
    library(sp)
    ...
    # ==============================================================================
    # 1. SIMULACIÓN DE DATOS ESPACIALES
    # ==============================================================================
    set.seed(1133)
    n <- 250
    coords <- cbind(runif(n, 0, 100), runif(n, 0, 100))
    
    # Parámetros verdaderos: Silla (sigmasq) = 5, Alcance (phi) = 20
    vero_pars <- c(sigmasq = 5, phi = 20) 
    D <- as.matrix(dist(coords))
    Sigma_verdadero <- vero_pars["sigmasq"] * exp(-D / vero_pars["phi"]) + diag(0.01, n)
    
    # Realización del proceso centrando en media 0
    Y <- mvrnorm(1, mu = rep(0, n), Sigma = Sigma_verdadero)
    
    # ==============================================================================
    # 2. FUNCIONES DE ESTIMACIÓN PERSONALIZADAS CON optim()
    # ==============================================================================
    
    # A. Variograma Empírico Muestral (para MCO y MCP)
    calc_vario_empirico <- function(coords, data, max_dist = 50, bins = 10) {
      d <- as.matrix(dist(coords))
      data_diff <- outer(data, data, "-")^2
      upper_idx <- upper.tri(d)
      
      d_vec <- d[upper_idx]
      diff_vec <- data_diff[upper_idx]
      
      mask <- d_vec <= max_dist & d_vec > 0
      d_vec <- d_vec[mask]
      diff_vec <- diff_vec[mask]
      
      breaks <- seq(0, max_dist, length.out = bins + 1)
      h_mids <- (breaks[-1] + breaks[-(bins+1)]) / 2
      gamma_hat <- numeric(bins)
      N_h <- numeric(bins)
      
      for(i in 1:bins) {
        idx <- d_vec > breaks[i] & d_vec <= breaks[i+1]
        N_h[i] <- sum(idx)
        gamma_hat[i] <- ifelse(N_h[i] > 0, mean(diff_vec[idx]) / 2, NA)
      }
      return(data.frame(h = h_mids, gamma = gamma_hat, N = N_h)[!is.na(gamma_hat), ])
    }
    
    v_emp <- calc_vario_empirico(coords, Y)
    
    # B. Funciones Objetivo para optim()
    # 1. MCO (OLS)
    obj_MCO <- function(pars, h, gamma_hat) {
      gamma_teo <- pars[1] * (1 - exp(-h / pars[2]))
      sum((gamma_hat - gamma_teo)^2)
    }
    
    # 2. MCP Ponderado por N(h)
    obj_MCP_npairs <- function(pars, h, gamma_hat, N_h) {
      gamma_teo <- pars[1] * (1 - exp(-h / pars[2]))
      sum(N_h * (gamma_hat - gamma_teo)^2)
    }
    
    # 3. MCP Ponderado por Varianza (Cressie)
    obj_MCP_var <- function(pars, h, gamma_hat, N_h) {
      gamma_teo <- pars[1] * (1 - exp(-h / pars[2]))
      weights <- N_h / (2 * (gamma_teo^2) + 1e-6)
      sum(weights * (gamma_hat - gamma_teo)^2)
    }
    
    # 4. Máxima Verosimilitud (ML)
    obj_ML <- function(pars, coords, Y) {
      Sigma <- pars[1] * exp(-D / pars[2]) + diag(1e-5, n)
      log_det <- as.numeric(determinant(Sigma, logarithm = TRUE)$modulus)
      quad_form <- as.numeric(t(Y) %*% solve(Sigma, Y))
      return(0.5 * (n * log(2 * pi) + log_det + quad_form))
    }
    
    # 5. Verosimilitud Restringida (REML)
    obj_REML <- function(pars, coords, Y) {
      X <- matrix(1, nrow = n, ncol = 1) # Modelo solo con intercepto
      Sigma <- pars[1] * exp(-D / pars[2]) + diag(1e-5, n)
      Sigma_inv <- solve(Sigma)
      
      P <- Sigma_inv - Sigma_inv %*% X %*% solve(t(X) %*% Sigma_inv %*% X) %*% t(X) %*% Sigma_inv
      log_det <- as.numeric(determinant(Sigma, logarithm = TRUE)$modulus)
      log_det_xt_sx <- as.numeric(determinant(t(X) %*% Sigma_inv %*% X, logarithm = TRUE)$modulus)
      
      quad_form <- as.numeric(t(Y) %*% P %*% Y)
      return(0.5 * (log_det + log_det_xt_sx + quad_form))
    }
    
    # 6. Pseudoverosimilitud (Composite Likelihood)
    obj_CL <- function(pars, coords, Y) {
      sigmasq <- pars[1]; phi <- pars[2]
      upper_idx <- upper.tri(D)
      h_vec <- D[upper_idx]
      u_ij <- outer(Y, Y, "-")[upper_idx]
      
      gamma_h <- sigmasq * (1 - exp(-h_vec / phi)) + 1e-6
      nll <- sum(0.5 * log(4 * pi * gamma_h) + (u_ij^2) / (4 * gamma_h))
      return(nll)
    }
    
    # ==============================================================================
    # 3. EJECUCIÓN DE OPTIMIZACIÓN CON optim()
    # ==============================================================================
    init_pars <- c(sigmasq = 5, phi = 10)
    
    opt_mco  <- optim(init_pars, obj_MCO, h = v_emp$h, gamma_hat = v_emp$gamma, method = "L-BFGS-B", lower = c(0.01, 0.01))
    opt_mcp1 <- optim(init_pars, obj_MCP_npairs, h = v_emp$h, gamma_hat = v_emp$gamma, N_h = v_emp$N, method = "L-BFGS-B", lower = c(0.01, 0.01))
    opt_mcp2 <- optim(init_pars, obj_MCP_var, h = v_emp$h, gamma_hat = v_emp$gamma, N_h = v_emp$N, method = "L-BFGS-B", lower = c(0.01, 0.01))
    opt_ml   <- optim(init_pars, obj_ML, coords = coords, Y = Y, method = "L-BFGS-B", lower = c(0.01, 0.01))
    opt_reml <- optim(init_pars, obj_REML, coords = coords, Y = Y, method = "L-BFGS-B", lower = c(0.01, 0.01))
    opt_cl   <- optim(init_pars, obj_CL, coords = coords, Y = Y, method = "L-BFGS-B", lower = c(0.01, 0.01))
    
    # ==============================================================================
    # 4. COMPARACIÓN CON PAQUETES ESTÁNDAR (geoR y gstat)
    # ==============================================================================
    
    # --- geoR ---
    # Solución al error de as.geodata: pasar una matriz/dataframe unificada
    datos_geoR <- as.geodata(cbind(coords, Y), coords.col = 1:2, data.col = 3)
    
    # 1. Ajuste con geoR
    fit_geoR_ml   <- likfit(datos_geoR, cov.model = "exponential", ini.cov.pars = c(5, 10), lik.method = "ML", messages = FALSE)
    fit_geoR_reml <- likfit(datos_geoR, cov.model = "exponential", ini.cov.pars = c(5, 10), lik.method = "REML", messages = FALSE)
    
    # 2. Ajuste MCO/MCP con geoR (variofit)
    fit_geoR_wls  <- variofit(variog(datos_geoR, max.dist = 50, messages = FALSE), cov.model = "exponential", ini.cov.pars = c(5, 10), weights = "cressie", messages = FALSE)
    
    # --- gstat ---
    df_sp <- SpatialPointsDataFrame(coords, data.frame(Y = Y))
    v_gstat <- variogram(Y ~ 1, df_sp, cutoff = 50)
    fit_gstat_wls <- fit.variogram(v_gstat, vgm(psill = 5, "Exp", range = 10, nugget = 0), fit.sills = TRUE)
    
    # ==============================================================================
    # 5. TABLA COMPARATIVA DE RESULTADOS
    # ==============================================================================
    res_tabla <- data.frame(
      Método = c("MCO (optim)", "MCP Pairs (optim)", "MCP Varianza (optim)", 
                 "ML (optim)", "REML (optim)", "Pseudoverosimilitud (optim)",
                 "ML (geoR)", "REML (geoR)", "MCP Cressie (geoR)", "WLS (gstat)"),
      `Silla (sigmasq)` = round(c(opt_mco$par[1], opt_mcp1$par[1], opt_mcp2$par[1], 
                                 opt_ml$par[1], opt_reml$par[1], opt_cl$par[1],
                                 fit_geoR_ml$cov.pars[1], fit_geoR_reml$cov.pars[1], 
                                 fit_geoR_wls$cov.pars[1], fit_gstat_wls$psill[2]), 2),
      `Alcance (phi)` = round(c(opt_mco$par[2], opt_mcp1$par[2], opt_mcp2$par[2], 
                                opt_ml$par[2], opt_reml$par[2], opt_cl$par[2],
                                fit_geoR_ml$cov.pars[2], fit_geoR_reml$cov.pars[2], 
                                fit_geoR_wls$cov.pars[2], fit_gstat_wls$range[2]), 2)
    )
    
    knitr::kable(res_tabla, col.names = c("Método de Estimación", "Silla (σ²)", "Alcance (φ)"),
                 caption = "Tabla 1. Comparación de parámetros estimados (Verdaderos: σ² = 5, φ = 20)",
                 align = "lcc") %>%
...
7. **Determine las dimensiones y la complejidad de cada uno de los métodos. ¿Cuál es la dimensión de las matrices invertidas en cada método? ¿En todos los casos el proceso es iterativo?**
    ...
    **R/**
    ...
    | Método | Matriz Invertida (Dimensión) | Complejidad Computacional | ¿Es Iterativo? | Justificación Teórica (Bohórquez, 2025) |
    | :--- | :--- | :--- | :--- | :--- |
    | **MCO** | No requiere inversión matricial directa (usa mínimos cuadrados no lineales). | **Baja** ($O(K \cdot p)$) | **No** (directo) | Al no haber matriz de pesos dependiente de $\Theta$, se resuelve directamente mediante algoritmos de optimización no lineal estándar. |
    | **MCP (Varianza)** | $K \times K$ (Matriz de pesos $W(\Theta)$) | **Media** ($O(K^3 \cdot \text{iter})$) | **Sí** | La matriz de ponderación $W(\Theta)$ depende de los parámetros $\Theta$. En cada iteración, se actualiza $\Theta$, se recalcula $W$, y se vuelve a minimizar. |
    | **MCP ($N(h)$)** | $K \times K$ (Matriz de pesos constante) | **Baja/Media** | **No** | Al usar $|N(h_k)|$, la matriz de pesos es constante y conocida *a priori*, eliminando la necesidad de iterar la estructura de pesos. |
    | **ML** | $n \times n$ (Matriz de covarianza $\Sigma(\Theta)$) | **Muy Alta** ($O(n^3 \cdot \text{iter})$) | **Sí** | Requiere calcular el determinante y la inversa de $\Sigma(\Theta)$ en cada paso de la optimización. Si $n > 10,000$, la memoria y el tiempo colapsan. |
    | **REML** | $n \times n$ (Matriz $A^\top \Sigma(\Theta) A$) | **Muy Alta** ($O(n^3 \cdot \text{iter})$) | **Sí** | Similar a ML, requiere inversión de matrices de dimensión $n \times n$ en cada iteración, aunque opera sobre el espacio de los incrementos. |
    | **CL** | $p \times p$ (Matriz Hessiana de Godambe) | **Baja** ($O(p^3 \cdot \text{iter})$) | **Sí** | La función $CL(\Theta)$ es una suma de escalares (log-verosimilitudes marginales). No requiere invertir matrices de $n \times n$. Solo se invierte la matriz de información de $p \times p$ para calcular errores estándar. |
    
    Mientras que los métodos basados en el variograma empírico (MCO, MCP) y la Pseudoverosimilitud (CL) operan en dimensiones reducidas ($K \times K$ o $p \times p$), haciéndolos escalables para grandes volúmenes de datos ($n$ grande); los métodos basados en la verosimilitud conjunta (ML, REML) están limitados por la inversión de la matriz de covarianza $n \times n$. Cuando $n$ es masivo, ML y REML se vuelven computacionalmente intratables, justificando el uso de CL o métodos jerárquicos.
...
Para garantizar la reproducibilidad y alinearnos con las matrices de dimensión $6 \times 6$ del caso de estudio, extraeremos las coordenadas de los primeros 6 puntos del dataset clásico `meuse` del paquete `sp`. 

Las relaciones fundamentales bajo estacionariedad de segundo orden son:

*   **Semivarianza:** $\gamma(h)$
*   **Covarianza:** $C(h) = C(0) - \gamma(h) = \sigma^2 - \gamma(h)$
*   **Correlación:** $\rho(h) = \frac{C(h)}{C(0)}$

```{r ej-3-4-datos, message=FALSE, warning=FALSE}
library(sp)
data(meuse)

# Tomamos los primeros 6 puntos para replicar la estructura del caso
coords <- meuse[1:6, c("x", "y")]
D <- as.matrix(dist(coords)) # Matriz de distancias euclidianas 6x6

# Variable de interés (ej. Zinc) para los cálculos de verosimilitud
Y <- meuse$zinc[1:6] 
n <- length(Y)
```
...
1. **Si el modelo es un exponencial con rango 50 y silla 1600, sin pepita.**
    ...
    **R/**
    ...
    La función de semivarianza es $\gamma(h) = 1600(1 - e^{-h/50})$. Al ser un modelo acotado, existen las tres matrices.
    
    ```{r ej_3_4_exp, message=FALSE, warning=FALSE}
    silla <- 1600; rango <- 50

    # Cálculos matriciales
    Gamma_exp <- silla * (1 - exp(-D / rango))
    Cov_exp   <- silla - Gamma_exp
    Rho_exp   <- Cov_exp / silla
    
    # Función auxiliar para formatear submatrices (primeras 5 filas y columnas)
    ver_submatriz <- function(mat, titulo) {
      sub_mat <- round(mat[1:min(5, nrow(mat)), 1:min(5, ncol(mat))], 2)
      kable(sub_mat, caption = paste(titulo, "(Vista previa 5x5)"), align = "c") %>%
        kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE)
    }
    
    # --- 1. Imprimir las matrices en formato tabla ---
    ver_submatriz(Gamma_exp, "Matriz de Semivarianza (γ)")
    ver_submatriz(Cov_exp,   "Matriz de Covarianza (C)")
    ver_submatriz(Rho_exp,   "Matriz de Correlación (ρ)")
    
    # --- 2. Resumen de Propiedades Matemáticas ---
    autovalores <- eigen(Cov_exp)$values
    es_simetrica <- isSymmetric(Cov_exp)
    es_def_pos   <- all(autovalores > 0)
    
    df_propiedades <- data.frame(
      Propiedad = c("Matriz Simétrica", "Definida Positiva", "Mín. Autovalor", "Máx. Autovalor"),
      Resultado = c(
        ifelse(es_simetrica, "Sí (Correcto)", "No"),
        ifelse(es_def_pos,   "Sí (Correcto)", "No"),
        round(min(autovalores), 4),
        round(max(autovalores), 4)
      )
    )
    
    kable(df_propiedades, caption = "Verificación de Propiedades de la Matriz de Covarianza", align = "lc") %>%
      kable_styling(bootstrap_options = c("striped", "bordered"), full_width = FALSE) %>%
      column_spec(2, bold = TRUE, color = ifelse(es_def_pos, "#2E7D32", "#C62828"))
    ```
    La matriz de covarianza es simétrica y todos sus valores propios son estrictamente positivos, garantizando varianzas de predicción positivas.
...
2. **Si el modelo es un seno cardinal con rango 50 y silla 1600, sin pepita.**
    ...
    **R/**
    ...
    La función es $\gamma(h) = 1600 \left[ 1 - \frac{\sin(h/50)}{h/50} \right]$ (con $\gamma(0)=0$). Este modelo presenta "efecto hueco" (correlaciones negativas).

    ```{r ej_3_4_sinc, message=FALSE, warning=FALSE}
    Gamma_sinc <- ifelse(D == 0, 0, silla * (1 - sin(D/rango)/(D/rango)))
    Cov_sinc   <- silla - Gamma_sinc
    Rho_sinc   <- Cov_sinc / silla
    
    # --- 1. Imprimir las matrices en formato tabla ---
    ver_submatriz(Gamma_sinc, "Matriz de Semivarianza (γ)")
    ver_submatriz(Cov_sinc,   "Matriz de Covarianza (C)")
    ver_submatriz(Rho_sinc,   "Matriz de Correlación (ρ)")
    
    # --- 2. Resumen de Propiedades Matemáticas ---
    autovalores <- eigen(Cov_sinc)$values
    es_simetrica <- isSymmetric(Cov_sinc)
    es_def_pos   <- all(autovalores > 0)
    ...
    kable(df_propiedades, caption = "Verificación de Propiedades de la Matriz de Covarianza", align = "lc") %>%
      ...
      column_spec(2, bold = TRUE, color = ifelse(es_def_pos, "#2E7D32", "#C62828"))
    ```
    Aunque permite correlaciones negativas (valores propios de $\rho(h)$ negativos), la matriz de covarianza global sigue siendo definida positiva, cumpliendo el Teorema de Bochner.
    
3. **Si el modelo es un cúbico con pendiente 50 y potencia 1600, sin pepita.**

    **R/**
    
    La función es $\gamma(h) = 1600 \left[ 7\left(\frac{h}{50}\right)^2 - \frac{35}{4}\left(\frac{h}{50}\right)^3 + \frac{7}{2}\left(\frac{h}{50}\right)^5 - \frac{3}{4}\left(\frac{h}{50}\right)^7 \right]$ para $h \le 50$, y $1600$ si $h > 50$.

```{r ej_3_4_cub, message=FALSE, warning=FALSE}
    h_norm <- D / rango
    # Fórmula exacta del variograma cúbico
    Gamma_cub <- ifelse(D <= rango, 
                        silla * (7*(h_norm^2) - (35/4)*(h_norm^3) + (7/2)*(h_norm^5) - (3/4)*(h_norm^7)), 
                        silla)
    Cov_cub   <- silla - Gamma_cub
    Rho_cub   <- Cov_cub / silla

    # --- 1. Imprimir las matrices en formato tabla ---
    ver_submatriz(Gamma_cub, "Matriz de Semivarianza (γ)")
    ver_submatriz(Cov_cub,   "Matriz de Covarianza (C)")
    ver_submatriz(Rho_cub,   "Matriz de Correlación (ρ)")
    
    # --- 2. Resumen de Propiedades Matemáticas ---
    autovalores <- eigen(Cov_cub)$values
    es_simetrica <- isSymmetric(Cov_cub)
    es_def_pos   <- all(autovalores > 0)
...
4. **Si el modelo es un potencial con pendiente 50 y potencia 1.6, sin pepita.**
    ...
    **R/**
    ...
    La función es $\gamma(h) = 50 h^{1.6}$. 

    ```{r ej_3_4_pot, message=FALSE, warning=FALSE}
    Gamma_pot <- 50 * D^1.6
    
    # --- 1. Imprimir las matrices en formato tabla ---
    ver_submatriz(Gamma_pot, "Matriz de Semivarianza (γ)")
    
    ```
    Las matrices de Covarianza $C(h)$ y Correlación $\rho(h)$ NO existen en este modelo. 
El modelo potencial es no acotado (crece indefinidamente con la distancia). Por lo tanto, no alcanza una meseta (silla), lo que implica que la varianza del proceso $Var[Z(s)] = C(0) \to \infty$. 

Este modelo solo satisface la estacionariedad intrínseca (de los incrementos), pero no la estacionariedad de segundo orden. Solo es válida la matriz de semivarianza $\Gamma_{pot}$.
...
5. **Calcule el valor de la función de verosimilitud según cada modelo y los datos dados. De acuerdo con estos resultados, ¿cuál de estos modelos es más adecuado para los datos?**
    ...
    **R/**
    ...
    Bajo normalidad multivariada, la ML solo es aplicable a modelos que poseen función de covarianza (estacionariedad de segundo orden). El modelo Potencial queda excluido de este criterio. La función a maximizar es:
$$ \ell(\Theta) = -\frac{n}{2}\ln(2\pi) - \frac{1}{2}\ln|\Sigma(\Theta)| - \frac{1}{2}(Y - \mu)^\top \Sigma^{-1}(\Theta) (Y - \mu) $$

    ```{r ej-3-4-ml, message=FALSE, warning=FALSE}
    # Función de Log-Verosimilitud Exacta usando Media GLS
calc_loglik <- function(Sigma, Y) {
  n <- length(Y)
  one <- rep(1, n)
  Sigma_inv <- solve(Sigma)
  
  # Estimador GLS de la media mu
  mu_gls <- as.numeric((t(one) %*% Sigma_inv %*% Y) / (t(one) %*% Sigma_inv %*% one))
  
  # Componentes de la Log-Verosimilitud
  det_sig <- as.numeric(determinant(Sigma, logarithm = TRUE)$modulus)
  quad_form <- as.numeric(t(Y - mu_gls) %*% Sigma_inv %*% (Y - mu_gls))
  
  log_lik <- -0.5 * (n * log(2 * pi) + det_sig + quad_form)
  return(log_lik)
}

LL_exp  <- calc_loglik(Cov_exp, Y)
LL_sinc <- calc_loglik(Cov_sinc, Y)
LL_cub  <- calc_loglik(Cov_cub, Y)

resultados_ml <- data.frame(
  Modelo = c("Exponencial", "Seno Cardinal", "Cúbico"),
  LogLik = round(c(LL_exp, LL_sinc, LL_cub), 2),
  Criterio = "Comparativo"
)

# Ahora LogLik existe perfectamente y la comparación no falla:
resultados_ml$Selección <- ifelse(resultados_ml$LogLik == max(resultados_ml$LogLik), 
                                 "Mejor Modelo", "")

# Imprimir con kable usando bonitos encabezados:
knitr::kable(
  resultados_ml, 
  col.names = c("Modelo", "Log-Verosimilitud (ℓ)", "Criterio", "Selección"),
  caption = "Comparación de Log-Verosimilitud Multivariada (ML)"
) %>%
...
6. **En un modelo de semivariograma anisotrópico, ¿cuál es la magnitud del rezago espacial en la dirección de máxima anisotropía? ¿Cómo queda el modelo de semivariograma en este caso?**
    ...
    **R/**
    ...
    En un modelo anisotrópico geométrico, la dependencia espacial varía según la dirección. Para modelarlo, se transforma el espacio original $(h_x, h_y)$ en un espacio isotrópico $(h'_\phi, h'_\theta)$ mediante una rotación y un reescalamiento.
    
    *   **Dirección de Máxima Anisotropía (Mayor Rango / Mayor Pendiente):**
        La magnitud del rezago espacial en esta dirección es la distancia original sin reescalar a lo largo del eje mayor de la elipse de anisotropía. Sin embargo, para evaluar el modelo isotrópico equivalente, el semivariograma se evalúa usando como alcance el rango mínimo ($a_\phi$). El modelo queda como $\gamma(h) \to \gamma'(\|h'\|)$, donde $\|h'\|$ comprime el eje mayor.
    
    Ell modelo de semivariograma anisotrópico se reduce a un modelo isotrópico evaluado sobre la distancia transformada $\|h'\| = \sqrt{(h'_\phi)^2 + (h'_\theta)^2}$, utilizando el rango de la dirección de menor anisotropía ($a_\phi$).
...
7. **En un modelo de semivariograma anisotrópico, ¿cuál es la magnitud del rezago espacial en la dirección de mínima anisotropía? ¿Cómo queda el modelo de semivariograma en este caso?**
    ...
    *   **Dirección de Mínima Anisotropía (Menor Rango / Menor Pendiente):**
        La magnitud del rezago es la distancia a lo largo del eje menor. El modelo isotrópico equivalente utiliza el rango mínimo ($a_\phi$) como su único parámetro de alcance, y la distancia transformada $\|h'\|$ expande el eje menor para igualarlo al mayor.
...
8. **Calcule el valor de la función de pseudoverosimilitud según cada modelo y los datos dados. De acuerdo con estos resultados, ¿cuál de estos modelos es más adecuado para los datos?**
    ...
    **R/**
    ...
    El método CL relaja el supuesto de normalidad conjunta y utiliza las marginales de los incrementos $U_{ij} \sim N(0, 2\gamma(h_{ij}))$. Al no requerir la inversión de la matriz de covarianza global $n \times n$, **es aplicable incluso al modelo Potencial**.

    ```{r ej-3-4-cl, message=FALSE, warning=FALSE}
    # Función para calcular la Log-Pseudoverosimilitud Compuesta
    calc_log_CL <- function(Gamma, Y) {
      # Extraer índices del triángulo superior (pares únicos i < j)
      idx <- upper.tri(Gamma)
      
      h_gamma <- Gamma[idx]
      # Matriz de diferencias Y_i - Y_j
      U_matrix <- outer(Y, Y, "-")
      U_ij <- U_matrix[idx]
      
      # Densidad marginal normal para cada par
      l_ij <- -0.5 * log(4 * pi * h_gamma) - (U_ij^2) / (4 * h_gamma)
      
      return(sum(l_ij))
    }
    
    # Cálculo para los 4 modelos
    CL_exp  <- calc_log_CL(Gamma_exp, Y)
    CL_sinc <- calc_log_CL(Gamma_sinc, Y)
    CL_cub  <- calc_log_CL(Gamma_cub, Y)
    CL_pot  <- calc_log_CL(Gamma_pot, Y)
    
    # Consolidación de resultados
    resultados_cl <- data.frame(
      Modelo = c("Exponencial", "Seno Cardinal", "Cúbico", "Potencial"),
      Log_CL = round(c(CL_exp, CL_sinc, CL_cub, CL_pot), 2),
      Criterio = "Maximización"
    )
    
    # Identificar el modelo óptimo (mayor valor de Log-CL)
    resultados_cl$Selección <- ifelse(resultados_cl$Log_CL == max(resultados_cl$Log_CL), 
                                     "Mejor Modelo", "")
    
    knitr::kable(
      resultados_cl, 
      col.names = c("Modelo", "Log-Pseudoverosimilitud (ℓ_CL)", "Criterio", "Selección"),
      caption = "Comparación de Log-Pseudoverosimilitud Compuesta (CL)"
    ) %>%
      kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
    ```
    
    Matemáticamente, el modelo Potencial puede arrojar el mayor valor de CL. Sin embargo, esto no lo hace el más adecuado teóricamente si el fenómeno exhibe estacionariedad de segundo orden (meseta). El método CL penaliza menos la falta de estructura de covarianza global. Si se sabe que el proceso es acotado, se debe preferir el modelo acotado con mejor CL (usualmente el Exponencial).
...
9. **Escriba cada modelo con la corrección necesaria en presencia de anisotropía.**
    ...
    **R/**
    ...
    Sea $\gamma(h; \Theta)$ el modelo isotrópico base. Ante anisotropía geométrica con razón $\lambda = \frac{a_\phi}{a_\theta} < 1$ y ángulo de máxima anisotropía $\theta$, se aplica la transformación de coordenadas $h \to h'$. Los modelos corregidos se escriben evaluando la norma transformada $\|h'\|$:

    1.  **Exponencial:** $\gamma(h') = 1600 \left( 1 - \exp\left( - \frac{\|h'\|}{50} \right) \right)$
    2.  **Seno Cardinal:** $\gamma(h') = 1600 \left[ 1 - \frac{\sin(\|h'\| / 50)}{\|h'\| / 50} \right]$
    3.  **Cúbico:** 
    $$ \gamma(h') = \begin{cases} 1600 \left[ 7\left(\frac{h}{50}\right)^2 - \frac{35}{4}\left(\frac{h}{50}\right)^3 + \frac{7}{2}\left(\frac{h}{50}\right)^5 - \frac{3}{4}\left(\frac{h}{50}\right)^7 \right] & \text{si } \|h'\| \le 50 \\ 1600 & \text{si } \|h'\| > 50 \end{cases} $$
    4.  **Potencial:** $\gamma(h') = 50 \|h'\|^{1.6}$
    
    Donde $\|h'\|$ es la distancia euclidiana en el sistema de coordenadas rotado y reescalado.
...
Se requiere llevar a cabo un análisis de anisotropía. Se detectó la existencia de una anisotropía de tipo geométrico cuya dirección principal es $25^\circ$ con un rango máximo de 105m y un rango mínimo de 35m.

1. **Escriba la matriz de rotación que usted usaría para incluir la anisotropía en el modelo de semivariograma.**

    **R/**
    
    De acuerdo con la Sección 3.8 del libro, cuando se detecta anisotropía geométrica, el primer paso es rotar los ejes coordenados originales $h = (h_x, h_y)$ hacia un nuevo sistema $h' = (h_\phi, h_\theta)$, donde el semivariograma se comporta de manera isotrópica. 

    El ángulo de máxima anisotropía es $\theta = 25^\circ$. Siguiendo la convención del texto, la matriz de rotación $R(\theta)$ que transforma las coordenadas cartesianas estándar al sistema alineado con los ejes de la elipse de anisotropía es:
    
    $$
    \begin{pmatrix} h_\phi \\ h_\theta \end{pmatrix} = 
    \begin{pmatrix} \cos(\theta) & \sin(\theta) \\ -\sin(\theta) & \cos(\theta) \end{pmatrix} 
    \begin{pmatrix} h_x \\ h_y \end{pmatrix}
    $$
    
    Sustituyendo $\theta = 25^\circ$:
    
    $$
    R(25^\circ) = \begin{pmatrix} \cos(25^\circ) & \sin(25^\circ) \\ -\sin(25^\circ) & \cos(25^\circ) \end{pmatrix} \approx \begin{pmatrix} 0.9063 & 0.4226 \\ -0.4226 & 0.9063 \end{pmatrix}
    $$
    
    ```{r ej-3-5-1-rotacion, message=FALSE, warning=FALSE}
# --- 1. Cálculo de la Matriz de Rotación ---
theta_grados <- 25
theta_rad <- theta_grados * (pi / 180)

matriz_rotacion <- matrix(c(
   cos(theta_rad),  sin(theta_rad),
  -sin(theta_rad),  cos(theta_rad)
), nrow = 2, byrow = TRUE)

rownames(matriz_rotacion) <- c("Eje Principal (h_φ)", "Eje Secundario (h_θ)")
colnames(matriz_rotacion) <- c("Eje X (h_x)", "Eje Y (h_y)")

# --- 2. Presentación Profesional en Tabla HTML / PDF ---
kable(
  round(matriz_rotacion, 4), 
  caption = "Matriz de Rotación R(25°) para Transformación Anisotrópica",
  align = "cc"
) %>%
  kable_styling(
  ...
  add_header_above(c(" " = 1, "Componentes Cartesianas" = 2))
    ```
...
2. **Describa cómo aplicaría la razón de anisotropía.**
    ...
    **R/**
    ...
    Una vez rotados los ejes, la elipse de anisotropía (que tiene un rango mayor $a_\theta = 105$ m y un rango menor $a_\phi = 35$ m) debe ser re-escalada para convertirla en un círculo de radio igual al menor rango de anisotropía ($a_\phi$). 

    Para lograr esto, se aplica la **razón de anisotropía** $\lambda$, definida como el cociente entre el rango menor y el rango mayor:
    
    $$
    \lambda = \frac{a_\phi}{a_\theta} = \frac{35}{105} = \frac{1}{3} \approx 0.3333
    $$
    
    El re-escalamiento se aplica únicamente sobre el eje de mayor anisotropía (el eje $\theta$) mediante una matriz diagonal de contracción. El vector de coordenadas transformadas finales $h'' = (h''_\phi, h''_\theta)$, que es el que realmente evaluará el modelo isotrópico, se obtiene así:
    
    $$
    \begin{pmatrix} h''_\phi \\ h''_\theta \end{pmatrix} = 
    \begin{pmatrix} 1 & 0 \\ 0 & \lambda \end{pmatrix} 
    \begin{pmatrix} h_\phi \\ h_\theta \end{pmatrix}
    $$
    
    De esta manera, la distancia anisotrópica transformada $\|h''\| = \sqrt{(h''_\phi)^2 + (h''_\theta)^2}$ permite que cualquier modelo isotrópico teórico (ej. esférico, exponencial) con rango $a_\phi = 35$ m sea evaluado correctamente en todas las direcciones.
    
    ```{r ej-3-5-2-razon, message=FALSE, warning=FALSE}
    # --- 1. Definición de Parámetros y Matriz ---
    a_theta <- 105 # Rango máximo
    a_phi   <- 35  # Rango mínimo
    
    lambda <- a_phi / a_theta
    
    matriz_escalamiento <- matrix(c(
      1, 0, 
      0, lambda
    ), nrow = 2, byrow = TRUE)
    
    rownames(matriz_escalamiento) <- c("Eje Referencia (h''_φ)", "Eje Escalado (h''_θ)")
    colnames(matriz_escalamiento) <- c("h_φ", "h_θ")
    
    # --- 2. Tabla Resumen del Parámetro λ ---
    df_lambda <- data.frame(
      Parametro = c("Rango Menor (a_φ)", "Rango Mayor (a_θ)", "Razón de Anisotropía (λ)"),
      Valor = c(paste(a_phi, "m"), paste(a_theta, "m"), round(lambda, 4))
    )
    
    kable(df_lambda, caption = "Parámetros para la Transformación de Anisotropía", align = "lc") %>%
      kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE)
    
    # --- 3. Presentación de la Matriz de Re-escalamiento ---
    kable(
      round(matriz_escalamiento, 4), 
      caption = "Matriz Diagonal de Re-escalamiento A",
      align = "cc"
    ) %>%
      kable_styling(
      ...
      add_header_above(c(" " = 1, "Ejes Rotados Previos" = 2))
    ```
...
3. **Ilustre a través de un caso particular.**
    ...
    **R/**
    ...
    Supongamos que tenemos dos puntos de muestreo separados por un vector de rezago espacial original $h = (h_x, h_y) = (40, 60)$ metros. 

    Para calcular el valor del semivariograma anisotrópico, debemos someter este vector a la transformación completa (Rotación + Re-escalamiento).
    
    **Paso 1: Rotación**
    $$
    \begin{pmatrix} h_\phi \\ h_\theta \end{pmatrix} = 
    \begin{pmatrix} 0.9063 & 0.4226 \\ -0.4226 & 0.9063 \end{pmatrix} 
    \begin{pmatrix} 40 \\ 60 \end{pmatrix} = 
    \begin{pmatrix} 61.61 \\ 37.48 \end{pmatrix} \text{ m}
    $$
    
    **Paso 2: Re-escalamiento**
    $$
    \begin{pmatrix} h''_\phi \\ h''_\theta \end{pmatrix} = 
    \begin{pmatrix} 1 & 0 \\ 0 & 0.3333 \end{pmatrix} 
    \begin{pmatrix} 61.61 \\ 37.48 \end{pmatrix} = 
    \begin{pmatrix} 61.61 \\ 12.49 \end{pmatrix} \text{ m}
    $$
    
    **Paso 3: Distancia Isotrópica Equivalente**
    La distancia que utilizará el modelo de semivariograma (cuyo rango práctico es ahora $a_\phi = 35$ m) es:
    $$
    \|h''\| = \sqrt{61.61^2 + 12.49^2} = \sqrt{3795.79 + 156.00} \approx 62.86 \text{ m}
    $$
    
    Como $\|h''\| = 62.86$ m es mayor que el rango de la esfera transformada ($35$ m), el modelo isotrópico equivalente indicará que estos dos puntos ya no presentan correlación espacial (la semivarianza habrá alcanzado la silla total).
    
    ```{r ej_3_5_3_caso, message=FALSE, warning=FALSE}
    # --- 1. Cálculos de la Transformación ---
    h_original <- c(40, 60)
    
    # Aplicar Rotación y Re-escalamiento
    h_rotado <- as.vector(matriz_rotacion %*% h_original)
    h_transformado <- as.vector(matriz_escalamiento %*% h_rotado)
    
    # Calcular distancia isotrópica equivalente
    distancia_iso <- sqrt(sum(h_transformado^2))
    
    # --- 2. Tabla de Transición del Vector h ---
    df_pasos <- data.frame(
      Etapa = c("1. Original (Cartesiano)", "2. Rotado (Ejes Elipse)", "3. Transformado (Isotrópico)"),
      Componente_1 = c(round(h_original[1], 2), round(h_rotado[1], 2), round(h_transformado[1], 2)),
      Componente_2 = c(round(h_original[2], 2), round(h_rotado[2], 2), round(h_transformado[2], 2)),
      Norma_Vectorial = c(
        paste(round(sqrt(sum(h_original^2)), 2), "m"),
        paste(round(sqrt(sum(h_rotado^2)), 2), "m"),
        paste(round(distancia_iso, 2), "m (h_eq)")
      )
    )
    
    kable(
      df_pasos, 
      col.names = c("Etapa de Transformación", "Eje X / h_φ", "Eje Y / h_θ", "Distancia Euclidiana"),
      caption = "Seguimiento Paso a Paso del Vector de Rezago h",
      align = "lccc"
    ) %>%
      kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE) %>%
      row_spec(3, bold = TRUE, background = "#E8F5E9") # Resaltar el resultado final
    ```
...
### Se requiere llevar a cabo predicción espacial para un proceso espacial cuya distribución marginal es lognormal con media conocida. 

¿Es posible encontrar un predictor insesgado? Demuestre o refute.
