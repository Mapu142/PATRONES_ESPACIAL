...
6. **Diseñe el código para que estos procesos de optimización, funcionen desde la tabla de los datos originales. Puede usar, por ejemplo, la función `optim` de R cran project. Compare los resultados obtenidos con su código con los obtenidos por las funciones de `geoR`, `variofit`, `likfit` o de `gstat`, `fit.variogram`.**
    ...
    **R/**
    ...
    ```{r paso_19_optimizacion_desde_cero, echo=TRUE, message=FALSE, warning=FALSE}
    # ==============================================================================
    # OPTIMIZACIÓN GEOESTADÍSTICA DESDE CERO CON optim() vs PAQUETES ESTÁNDAR
    # ==============================================================================
    library(gstat)
    library(geoR)
    library(MASS) # Para mvrnorm
    
    # 1. Simulación de un conjunto de datos espacial (Proceso Exponencial)
    set.seed(123)
    n <- 50
    coords <- cbind(runif(n, 0, 100), runif(n, 0, 100))
    # Parámetros verdaderos: Silla (sigmasq) = 10, Alcance (phi) = 20
    vero_pars <- c(sigmasq = 10, phi = 20) 
    Sigma_verdadero <- vero_pars["sigmasq"] * exp(-as.matrix(dist(coords)) / vero_pars["phi"])
    Sigma_verdadero <- Sigma_verdadero + diag(0.01, n) # Efecto pepita pequeño para invertibilidad
    Y <- mvrnorm(1, mu = rep(0, n), Sigma = Sigma_verdadero)
    
    # ------------------------------------------------------------------------------
    # A. ENFOQUE BASADO EN VARIOGRAMA: MCO usando optim()
    # ------------------------------------------------------------------------------
    # Función para calcular el variograma empírico (binning simple)
    calc_vario_empirico <- function(coords, data, max_dist = 50, bins = 10) {
      ...
      return(list(h = h_mids, gamma = gamma_hat, N = N_h))
    }
    
    v_emp <- calc_vario_empirico(coords, Y)
    validos <- !is.na(v_emp$gamma)
    
    # Función objetivo MCO
    obj_MCO <- function(pars, h, gamma_hat) {
      sigmasq <- pars[1]; phi <- pars[2]
      gamma_teo <- sigmasq * (1 - exp(-h / phi))
      sum((gamma_hat - gamma_teo)^2)
    }
    
    # Optimización MCO
    opt_MCO <- optim(par = c(5, 10), fn = obj_MCO, h = v_emp$h[validos], 
                     gamma_hat = v_emp$gamma[validos], method = "L-BFGS-B", 
                     lower = c(0.01, 0.01))
    cat("Parámetros MCO (optim): sigmasq =", round(opt_MCO$par[1], 2), 
        ", phi =", round(opt_MCO$par[2], 2), "\n")
    
    # ------------------------------------------------------------------------------
    # B. ENFOQUE BASADO EN VEROSIMILITUD: ML usando optim()
    # ------------------------------------------------------------------------------
    # Función objetivo ML (Negativo de la Log-Verosimilitud)
    obj_ML <- function(pars, coords, Y) {
      sigmasq <- pars[1]; phi <- pars[2]
      D <- as.matrix(dist(coords))
      Sigma <- sigmasq * exp(-D / phi)
      Sigma <- Sigma + diag(1e-6, nrow(Sigma)) # Jitter para estabilidad numérica
      
      # Cálculo de la log-verosimilitud multivariada normal
      term1 <- (n / 2) * log(2 * pi)
      term2 <- 0.5 * determinant(Sigma, logarithm = TRUE)$modulus
      term3 <- 0.5 * t(Y) %*% solve(Sigma) %*% Y
      
      return(as.numeric(term1 + term2 + term3))
    }
    
    # Optimización ML
    opt_ML <- optim(par = c(5, 10), fn = obj_ML, coords = coords, Y = Y, 
                    method = "L-BFGS-B", lower = c(0.01, 0.01))
    cat("Parámetros ML (optim): sigmasq =", round(opt_ML$par[1], 2), 
        ", phi =", round(opt_ML$par[2], 2), "\n")
    
    # ------------------------------------------------------------------------------
    # C. COMPARACIÓN CON PAQUETES ESTÁNDAR (geoR y gstat)
    # ------------------------------------------------------------------------------
    # Preparar datos para geoR
    geodata <- as.geodata(coords, data = Y)
    
    # Ajuste con geoR (Máxima Verosimilitud)
    fit_geoR <- likfit(geodata, ini.cov.pars = c(5, 10), cov.model = "exponential", 
                       lik.method = "ML")
    cat("Parámetros ML (geoR): sigmasq =", round(fit_geoR$sigmasq, 2), 
        ", phi =", round(fit_geoR$phi, 2), "\n")
    
    # Preparar datos para gstat
    library(sf)
    df_sf <- st_as_sf(data.frame(coords, Y), coords = c("X1", "X2"))
    g_obj <- gstat(formula = Y ~ 1, data = df_sf)
    v_gstat <- variogram(g_obj)
    
    # Ajuste con gstat (Mínimos Cuadrados Ponderados por defecto)
    fit_gstat <- fit.variogram(v_gstat, vgm(psill = 10, model = "Exp", range = 20, nugget = 0))
    cat("Parámetros MCP (gstat): sigmasq =", round(fit_gstat$psill[2], 2), 
        ", phi =", round(fit_gstat$range[2], 2), "\n")
    ```
