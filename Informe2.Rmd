...
# ==============================================================================
# VALIDACIÓN CRUZADA (Leave-One-Out) con Regularización de Matriz
# ==============================================================================
# 1. Extraer los parámetros estimados por su modelo REML (fitvar5)
cov_pars_reml <- fitvar5$cov.pars   # Contiene sigmasq (silla parcial) y phi (rango)
beta_reml     <- fitvar5$beta       # Contiene los coeficientes de la tendencia (1st)

# 2. Ejecutar xvalid pasando los parámetros manualmente y forzando un nugget mínimo
# Un nugget de 1e-5 es suficiente para estabilizar la matriz sin alterar la predicción
cv_result <- xvalid(datos_geoR, 
                    trend = "1st", 
                    beta = beta_reml,
                    cov.model = "exponential", 
                    cov.pars = cov_pars_reml, 
                    nugget = 1e-5) # <- El "parche" matemático para evitar la singularidad
...
plot(cv_result$data, cv_result$predicted, 
     xlab = "Observado (Residuales)", ylab = "Predicho (CV)",
     main = "Validación Cruzada (Leave-One-Out)",
     pch = 19, col = "darkblue")
abline(0, 1, col = "red", lwd = 2) # Línea de identidad (1:1)

# Gráfico de Residuos de la CV (Error de predicción)
plot(cv_result$data, cv_result$data - cv_result$predicted,
     xlab = "Observado", ylab = "Error de Predicción (Obs - Pred)",
     main = "Residuos de la Validación Cruzada",
     pch = 19, col = "darkgreen")
abline(h = 0, col = "red", lwd = 2, lty = 2)
par(mfrow = c(1, 1))

# Calcular el Error Cuadrático Medio (RMSE)
rmse <- sqrt(mean((cv_result$data - cv_result$predicted)^2))
cat("Raíz del Error Cuadrático Medio (RMSE) de la CV:", round(rmse, 4), "\n")
```
