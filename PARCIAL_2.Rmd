...
La calidad del aire es un tema que ha venido cogiendo fuerza y en ciudades suele ser más delicado, hay distintos contaminantes desde $O_3$ que es .... hasta $PM_{10}$ que es ...
...
La base de datos fue construida a partir de la unión de varios datasets debido a que la página solo permitía descargar maximo 6 meses y una variable. Al final consta de 13243 observaciones (19 estaciones $\times$ 697 tiempos).
...
Cuenta con 1911 datos faltantes en la variable de interés $PM_{10}$ (véase la Tabla \@ref(tab:faltantes)). Además, en la Figura \@ref(fig:boxplot-pm10) se observa que la mayor parte de las concentraciones se encuentran aproximadamente entre 1 y 60 $μg/m^3$, aunque también se evidencian numerosos valores atípicos y algunos valores extremadamente altos. Esta presencia de valores extremos sugiere una distribución asimétrica y una alta variabilidad en los datos, lo que podría influir en la estimación del variograma y, en consecuencia, en el desempeño del kriging, al incrementar la sensibilidad de estos procedimientos a observaciones de gran magnitud.
...
Podemos ver en la Figura \@ref(fig:mapa-bogota) que la distribución espacial de las estaciones de monitoreo no es homogénea, concentrándose principalmente en la zona urbana de Bogotá. Por otra parte, en el gif \@ref(fig:anim-bogota) se observa que, a lo largo del período de estudio, la mayoría de las estaciones presentan concentraciones de $PM_{10}$ dentro de rangos similares, evidenciado por la predominancia de colores y tamaños semejantes entre los puntos. Los episodios de concentraciones elevadas son poco frecuentes y se presentan únicamente en algunos instantes y estaciones específicas, lo que es consistente con la presencia de unos pocos valores extremos observados previamente en el diagrama de caja.


## Mapa de bogotá


```{r mapa-bogota, fig.cap="Visalización ed las 19 estaciones de Bogotá"}
ggplot() +
  geom_sf(
    data = bogota,
    fill = "gray95",
...
```{r anim-bogota, fig.cap="Gif de las observaciones a través del tiempo", echo=FALSE, results='asis'}
...
+ La Figura \@ref(fig:correlacion-variable) muestra que no se evidencia una correlación lineal entre la concentración de $PM_{10}$ y las variables geográficas Altitud (m) y Altura (m), con coeficientes de correlación de 0.04 y 0.03, respectivamente. Esto sugiere que, al menos desde una perspectiva lineal, la concentración del contaminante no presenta una relación directa con la elevación del terreno ni con la altura de la estación de monitoreo, por lo que otros factores ambientales o antrópicos podrían desempeñar un papel más importante en su variabilidad espacial.

+ En la Figura \@ref(fig:dispm10lon) así como también en la figura \@ref(fig:disPM10lat) no se observa una tendencia clara entre la concentración de $PM_{10}$ y la longitud. La curva suavizada mediante LOESS permanece prácticamente horizontal a lo largo del dominio espacial, lo que sugiere que el valor esperado de la concentración no presenta un cambio sistemático en función de la longitud. Asimismo, se aprecia una alta dispersión de las observaciones para prácticamente todos los valores de longitud, lo que indica que esta coordenada, por sí sola, no explica una proporción importante de la variabilidad observada en las concentraciones.
...
```{r correlacion-variable, fig.height=10, fig.width=10, fig.cap="Correlación lineal entre algunas variables"}
corrplot(
  mat_cor,
  method = "color",
  addCoef.col = "black",
