...
# Alicación Práctica en R: Análisis de Patrones Puntuales de Sismicidad en Colombia
...
Para el análisis de patrones puntuales, la librería principal es **`spatstat`**. Adicionalmente, utilizaremos **`sf`** para el manejo moderno de datos espaciales y **`readxl`** para importar desde Excel.
...
df <- df %>% select(Latitud, Longitud, Magnitud) %>%
  mutate(
...
ggplot() +
  ...
  # Capa 2: Puntos Sísmicos (Mapeamos color y tamaño según la Magnitud)
...
Un patrón puntual espacial consiste en un conjunto de ubicaciones llamadas **eventos**. Para usar `spatstat`, debemos extraer las coordenadas y definir una **ventana de observación** (`owin`), que delimita el área donde el fenómeno es posible.
...
$$\chi^2 = 374.33 \quad \vert{} \quad \text{grados de libertad (df)} = 24 \quad \vert{} \quad \text{p-value} < 2.2 \times 10^{-16}$$ 
Dado que el p-value es críticamente menor al nivel de significancia ($\alpha = 0.05$ ), existe evidencia estadística para rechazar la hipótesis nula ($H_0$). Se concluye que los sismos en Colombia y su área de influencia no ocurren de manera aleatoria u homogénea en el espacio.
...
mosaico_cuadrantes <- (g_4x4 + g_5x5) + 
  plot_annotation(
...
Debido a que la cuadrícula de 5x5 presenta cuadrantes periféricos con bajas frecuencias esperadas ($E = 3.4$) y celdas con ausencia absoluta de eventos ($O = 0$), la aproximación asintótica tradicional de la prueba Chi-cuadrado podría perder validez. Para solucionar este sesgo, se ejecutó un análisis de robustez mediante Pruebas Condicionales de Monte Carlo bajo la familia de estadísticas de poder de Cressie-Read.
