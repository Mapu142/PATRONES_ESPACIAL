...
$$\chi^2 = 374.33 \quad \vert{} \quad \text{grados de libertad (df)} = 24 \quad \vert{} \quad \text{p-value} < 2.2 \times 10^{-16}$$ Dado que el p-value es críticamente menor al nivel de significancia ($\alpha = 0.05$ ), existe evidencia estadística para rechazar la hipótesis nula ($H_0$).
...
El motivo por el cual seleccionamos una rejilla de $3\times 3$ cuadrantes disjuntos se ilustra en la figura \@ref(fig:mosaico). Aunque con esta partición el valor esperado por celda es $E=0.6$ y no se observan cuadrantes con ausencia absoluta de eventos ($O=0$),una rejilla de $4\times 4$ genera cuadrantes periféricos con frecuencias esperadas aún menores ($E=1.6$) y celdas con ausencia absoluta de eventos. 
...
```{r chi_robusto, message=FALSE, warning=FALSE}
# ==============================================================================
# 1. CONSOLIDACIÓN DE LOS RESULTADOS EN UN DATAFRAME
# ==============================================================================
(prueba_chi_2 <- quadrat.test(sismos_ppp,method="MonteCarlo",CR=-2, nx = 4, ny = 4))
(prueba_chi_1_error <- quadrat.test(sismos_ppp,method="MonteCarlo",CR=-1, nx = 4, ny = 4))
(prueba_chi_05 <- quadrat.test(sismos_ppp,method="MonteCarlo",CR=-1/2, nx = 4, ny = 4))
(prueba_chi_1_3 <- quadrat.test(sismos_ppp,method="MonteCarlo",CR=1/3, nx = 4, ny = 4))
(prueba_chi_2_3 <- quadrat.test(sismos_ppp,method="MonteCarlo",CR=2/3, nx = 4, ny = 4))
(prueba_chi_1 <- quadrat.test(sismos_ppp,method="MonteCarlo",CR=1, nx = 4, ny = 4))
...
# ==============================================================================
# 2. RENDERIZADO DE LA TABLA ACADÉMICA CON KABLEEXTRA
# ==============================================================================
knitr::kable(
  tabla_resultados,
  caption = "Tabla 1. Pruebas de Robustez de Monte Carlo bajo la Familia de Cressie-Read (Rejilla 5x5)",
  col.names = c("Estadístico de Prueba", "Parámetro de Poder", "Valor del Test", "p-value (Simulado)", "Diagnóstico Espacial"),
  align = c("l", "c", "c", "c", "l")
) %>%
  ...
  kableExtra::row_spec(c(3, 5), bold = TRUE, background = "#e8f4fd") %>%      
  kableExtra::row_spec(2, color = "#7f8c8d", italic = TRUE) %>%               
  kableExtra::footnote(
    general = " ",
    general_title = "Simulaciones condicionales de Monte Carlo basadas en un número fijo de eventos. Nivel de significancia alfa = 0.05. El estadístico con parámetro CR = -1.0 diverge debido a la presencia de cuadrantes periféricos vacíos (Frecuencia Observada = 0)."
  )
```

Los resultados confirman que al evaluar el patrón mediante el estadístico óptimo de Cressie-Read la sismicidad mantiene un rechazo absoluto del modelo nulo de aleatoriedad.

Al evaluar los vecinos de primer y segundo orden presentados en la tabla 2.
