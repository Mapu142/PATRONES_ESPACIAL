...
```{r Exploratoria, fig.cap="Visualización de sismos en Colombia en el año 1990", message=FALSE, warning=FALSE}
# ==============================================================================
...
```{r owin, fig.cap="Patrón puntual sísmico e influencia regional"}
ggplot() +
...
$$\chi^2 = 26.047 \quad \vert{} \quad \text{grados de libertad (df)} = 8 \quad \vert{} \quad \text{p-value} < 2.06 \times 10^{-3}$$ Dado que el p-value es críticamente menor al nivel de significancia ($\alpha = 0.05$ ), existe evidencia estadística para rechazar la hipótesis nula ($H_0$).
...
(chi_4x4 <- quadrat.test(sismos_ppp, nx = 4, ny = 4))
(chi_5x5 <- quadrat.test(sismos_ppp, nx = 5, ny = 5)) 
(chi_6x6 <- quadrat.test(sismos_ppp, nx = 6, ny = 6)) 
...
(prueba_chi_1_error <- quadrat.test(sismos_ppp,method="MonteCarlo",CR=-1, nx = 3, ny = 3))
(prueba_chi_05 <- quadrat.test(sismos_ppp,method="MonteCarlo",CR=-1/2, nx = 3, ny = 3))
(prueba_chi_1_3 <- quadrat.test(sismos_ppp,method="MonteCarlo",CR=1/3, nx = 3, ny = 3))
(prueba_chi_2_3 <- quadrat.test(sismos_ppp,method="MonteCarlo",CR=2/3, nx = 3, ny = 3))
(prueba_chi_1 <- quadrat.test(sismos_ppp,method="MonteCarlo",CR=1, nx = 3, ny = 3))
...
CE_sismos <- clarkevans.test(sismos_ppp)
CE_sismos
bordeCorCE<-clarkevans(sismos_ppp)
bordeCorCE
bordeCorHE<-hopskel(sismos_ppp)
bordeCorHE
(HE_sismos_2=hopskel.test(sismos_ppp))
(HE_sismos_clust=hopskel.test(sismos_ppp,alternative="clustered"))
(HE_sismos_reg=hopskel.test(sismos_ppp,alternative="regular"))
...
# 1. Convertir los resultados de spatstat a Data Frames
df_kest <- as.data.frame(Kest(sismos_ppp))
df_env  <- as.data.frame(envK)

# 2. Construir el primer gráfico (Kest)
g1 <- ggplot(df_kest, aes(x = r)) +
  geom_line(aes(y = iso, color = "Isotrópica")) +
  geom_line(aes(y = trans, color = "Translación")) +
  geom_line(aes(y = border, color = "Borde")) +
  geom_line(aes(y = theo, color = "Teórica"), linetype = "dashed") +
  labs(
    title = "Función K de Ripley con diferentes\ncorrecciones de borde",
    x = "Distancia (r)", y = "K(r)", color = "Corrección"
  ) +
  theme_minimal() +
  theme(plot.title = element_text(size = 10, face = "bold", hjust = 0.5))

# 3. Construir el segundo gráfico (Envelope / Bandas de confianza)
...
# 4. Combinar ambos gráficos sin errores
mosaico3 <- g1 + g2
```

```{r kest, fig.cap="Estimación de la función K de Ripley para los sismos en Colombia utilizando las correcciones isotrópica, de traslación y de borde. La curva teórica K pois(r) corresponde al proceso de Poisson homogéneo (CSR)"}
plot(mosaico3)
```



Las distintas correciones mostradas en el gráfico \@ref(fig:kest) conducen a la misma interpretación general: la función $K(r)$ observada se encuentra por encima de la curva teórica esperada bajo un proceso de Poisson homogéneo, lo que sugiere una mayor concentración de eventos de la esperada bajo aleatoriedad espacial completa. No obstante, este resultado puede estar influenciado por la presencia de variaciones espaciales en la intensidad de ocurrencia de los sismos, ya que existen zonas con una mayor densidad de eventos que otras. Dado que la función K clásica supone homogeneidad espacial, a continuación se emplea la función *K inhomogénea* (**Kinhom**), la cual incorpora una estimación de la intensidad local y permite evaluar la estructura espacial de segundo orden una vez controlada la heterogeneidad de primer orden.

```{r}
lambda_s <- density(sismos_ppp)
K_sismos_NH <- Kinhom(
  sismos_ppp,
  lambda_s,
  correction = "translate"
...
El gráfico \@ref(fig:kinhom) muestra que la aparente agrupación observada mediante la función K clásica parece explicarse principalmente por la variación espacial de la intensidad de ocurrencia de los sismos. Una vez controlada dicha heterogeneidad mediante la función K inhomogénea, la evidencia de agregación disminuye considerablemente.
...
La función de correlación por pares, mostrada en el gráfico \@ref(fig:pcfkinhom), presenta valores superiores a la unidad para distancias cortas, lo que sugiere la posible existencia de agregación residual entre eventos incluso después de corregir la heterogeneidad espacial mediante un modelo inhomogéneo.

```{r}
set.seed(1250)
# 1. Calcular las bandas de confianza usando Fest
Fenvel <- envelope(sismos_ppp, Fest, nsim = 499)

# 2. Convertir los resultados de spatstat a Data Frames
df_Fest <- as.data.frame(Fest(sismos_ppp))
df_env  <- as.data.frame(Fenvel) 

# 3. Construir el primer gráfico (Fest con diferentes correcciones)
g1 <- ggplot(df_Fest, aes(x = r)) +
  geom_line(aes(y = cs, color = "Sin corrección")) +
  geom_line(aes(y = rs, color = "Borde (Border)")) +
  geom_line(aes(y = km, color = "Kaplan-Meier")) +
  geom_line(aes(y = theo, color = "Teórica"), linetype = "dashed") +
  labs(
    title = "Función F de Ripley con diferentes\ncorrecciones de borde",
    x = "Distancia (r)", 
    y = "F(r)", # Cambiado a F(r)
    color = "Corrección"
  ) +
  theme_minimal() +
  theme(plot.title = element_text(size = 10, face = "bold", hjust = 0.5))

# 4. Construir el segundo gráfico (Envelope de la función F)
g2 <- ggplot(df_env, aes(x = r)) +
  # Sombra gris para las bandas de confianza (mínimo y máximo simulado)
  geom_ribbon(aes(ymin = lo, ymax = hi), fill = "grey80", alpha = 0.5) +
  # Línea teórica esperada
  geom_line(aes(y = theo), color = "red", linetype = "dashed") +
  # Línea observada en tus datos reales (por defecto usa la mejor corrección)
  geom_line(aes(y = obs), color = "black", linewidth = 0.8) +
  labs(
    title = "Bandas de confianza para\nla función F de Ripley",
    x = "Distancia (r)", 
    y = "F(r)" # Cambiado a F(r)
  ) +
  theme_minimal() +
...
```{r Fest, fig.cap="Estimación de la función K de Ripley para los sismos en Colombia utilizando distintas correcciones"}
plot(mosaico4)
```

```{r}
plot(Jest(sismos_ppp))
```

```{r}
plot(Gest(sismos_ppp))
```



```{r}
plot(Hest(sismos_ppp))
```
